...
| 한국어 펌웨어 | **3.11-ko.1** |
| 검증된 코드·펌웨어 커밋 | `af410d08fd8c470e9f57043ce1839e40946710f4` |
...
현재 인수인계 문서 커밋은 위 펌웨어 커밋 이후의 기록 추가입니다. 문서 추가로 펌웨어 버전이나 바이너리가 바뀌지는 않습니다.
...
- [기본 브랜치 GitHub 전체 검사](https://github.com/Loaram/TamaPoke_ko/actions/runs/33709498914): 성공.
- [GitHub Pages 배포](https://github.com/Loaram/TamaPoke_ko/actions/runs/33709498902): 성공.
- 한글 글꼴·문자열·이름·표시 폭, 세이브 복원, 이전 저장 형식 업그레이드, 통신 프로토콜 회귀 검사 통과.
- 독립 에뮬레이터 프로세스를 재시작해도 한국어 설정 유지. 실제 에뮬레이터 화면 12개 확인.
- 공개 HTTPS 페이지의 한국어 설치 버튼 활성화와 v3.11-ko.1 표시 확인. 펌웨어 4개 파일의 SHA-256이 로컬 빌드와 일치. 7개 팩 HTTP 응답과 크기 확인.
