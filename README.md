...
현재 3.7.3 한국어 화면을 넣은 **29쪽 초보자용 플레이 설명서**는 [설치 페이지의 설명서 다운로드](https://loaram.github.io/TamaPoke_ko/#play-guide)에서 PDF 또는 페이지별 이미지로 받을 수 있습니다. 탐색의 조우 레벨·종류 확률과 포획 확률은 11~12쪽, 기기 조합별 근거리 배틀은 13~15쪽, 세이브 이전은 16~18쪽, 폼·연속 돌봄·도감 보너스·자유 교체는 19~22쪽, 박스 정렬은 23쪽, 동료 성장·개체값은 24쪽, 탐색 결과 확인창은 25쪽에서 설명합니다.
...
- 기존 6개 언어와 1.1.6 이하 저장을 자동 변환해 유지합니다. 3.7.3의 무선 규격은 프로토콜 5이며 세이브 전송에는 현재 포켓몬, 파티 5마리, 300칸 박스, 도감, 배지와 설정이 포함됩니다.
...
결과는 `build/android/TamaPoke-3.7.3-Android-Full-debug.apk`이며 Android 기기용 ARM64, 에뮬레이터용 x86_64와 9개 지역 팩을 포함합니다. 성장·시계와 배터리 표시는 Android 설정과 실제 잔량을 직접 따릅니다.
...
결과는 `build/android/TamaPoke-3.7.3-WearOS-GalaxyWatch4-9-debug.apk`입니다. Galaxy Watch4부터 Watch9까지 대응하도록 32비트 `armeabi-v7a`와 64비트 `arm64-v8a`를 함께 넣었으며, 워치가 자신에게 맞는 실행 파일을 자동 선택합니다. 실제 원형 화면 해상도에 맞춰 화면과 터치 좌표를 자동 조정하고 9개 지역 팩을 포함하는 독립형 Wear OS 앱입니다. 성장·시계와 배터리 표시는 워치의 날짜·시간 및 실제 잔량을 따릅니다. LAN 대전과 세이브 전송은 워치 Wi-Fi를 휴대전화와 같은 공유기 또는 실기의 `TamaPoke-XXXX` 방에 연결해 사용합니다.
