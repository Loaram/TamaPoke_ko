...
**3.7.0 공용 활력:** 탐색 비용은 22이며 포획한 포켓몬이나 파티·박스 동료를 데려와도 활력이 유지됩니다. 새 알로도 충전되지 않습니다. 수면 회복은 기존 8/분이며 기존 세이브의 현재 활력을 이어받습니다.
...
**정식 3.8.0** · 애니메이션 지원 폼 176개, 폼별 타입·능력치·기술, 가라르·팔데아 체육관, 자유 교체, 300칸 박스와 연속 돌봄 보너스를 제공합니다. [릴리스 안내](docs/RELEASE-3.8.0.ko.md) · [최신 플레이 가이드](web/guides/TamaPoke-3.8.0-Play-Guide-KO.pdf)

**업데이트 전 세이브를 백업하세요.** 저장 v7은 이전 v1~v6을 읽지만 구버전으로 되돌릴 수 없습니다. 근거리 연결은 양쪽을 3.8.0로 맞추세요. **ESP는 300칸 보관 기록용 microSD가 필수**이며 기존 SD와 내부 저장을 함께 유지해야 합니다. 앱에는 9개 지방과 폼 그림 팩이 포함됩니다.
...
버전 규칙은 `a.b.c`이며 주요 기능·대규모 변경은 a, 편의 기능·기능 수정·밸런스 패치는 b, 텍스트·간단한 패치는 c를 올립니다. 상위 번호가 바뀌면 하위 번호는 0으로 초기화합니다.
...
![펌웨어](https://img.shields.io/badge/펌웨어-3.8.0-8A2BE2)
...
현재 정식 버전은 **3.8.0**입니다. 지방과 탐색 방식을 골라 야생 포켓몬과 싸우고 승리 뒤 포획하는 탐색 시스템을 추가했습니다. 조우 1회당 공용 활력 22를 사용하며 일반은 현재 포켓몬 레벨 ±5, 랜덤은 레벨 1~100에서 상대가 정해집니다. 파티는 현재 키우는 포켓몬 1마리와 보관 파티 5마리를 합쳐 전투 후보가 최대 6마리이며, 300칸/50페이지 박스를 사용하며 이전 세이브를 자동 변환합니다. Android·Wear OS의 배터리 표시는 실제 기기 잔량과 충전 상태를 따릅니다.

현재 3.8.0 한국어 화면을 넣은 **29쪽 초보자용 플레이 설명서**는 [설치 페이지의 설명서 다운로드](https://loaram.github.io/TamaPoke_ko/#play-guide)에서 PDF 또는 페이지별 이미지로 받을 수 있습니다. 탐색의 조우 레벨·종류 확률과 포획 확률은 11~12쪽, 기기 조합별 근거리 배틀은 13~15쪽, 세이브 이전은 16~18쪽, 폼·연속 돌봄·도감 보너스·자유 교체는 19~22쪽, 박스 정렬은 23쪽, 동료 성장·개체값은 24쪽, 탐색 결과 확인창은 25쪽에서 설명합니다.

박스에서 포켓몬을 누르면 항상 상세창이 열리며 **놓아주기 → 예**로 바로 정리할 수 있습니다. 교체 대상을 먼저 골랐더라도 상세창의 **교체** 버튼을 눌러 확정합니다. 파티/박스 놓아주기는 알 수령 횟수를 사용하거나 되돌려주지 않습니다.
...
포획·보관 동료도 **데려오기**로 홈에 오면 20분마다 1레벨씩, 최대 100레벨까지 성장하고 조건을 갖추면 일반 진화합니다. 보관 중에는 멈추며 기존 도망 방지 보호를 유지합니다. 이 동료를 키우는 중에도 새 알을 받을 수 있습니다. 파티/박스 상세창의 **공격 99(31)**에서 괄호 안 숫자는 **개체값(0~31)**입니다.
...
- UI 문자열 194개, 포켓몬 1025종 이름, 기술 테이블 717개(빈 기술 포함 718항목), 18개 타입, 지방·트레이너·장소 표시.
- 메뉴, 도감, 상태, 성장, 기술 선택, 배틀, 파티, 박스, 체육관, 근거리 대전·세이브 전송, 언어 설정.
- 기존 6개 언어와 1.1.6 이하 저장을 자동 변환해 유지합니다. 3.8.0의 무선 규격은 프로토콜 5이며 세이브 전송에는 현재 포켓몬, 파티 5마리, 300칸 박스, 도감, 배지와 설정이 포함됩니다.
...
결과는 `build/android/TamaPoke-3.8.0-Android-Full-debug.apk`이며 Android 기기용 ARM64, 에뮬레이터용 x86_64와 9개 지역 팩을 포함합니다. 성장·시계와 배터리 표시는 Android 설정과 실제 잔량을 직접 따릅니다.
...
결과는 `build/android/TamaPoke-3.8.0-WearOS-GalaxyWatch4-9-debug.apk`입니다. Galaxy Watch4부터 Watch9까지 대응하도록 32비트 `armeabi-v7a`와 64비트 `arm64-v8a`를 함께 넣었으며, 워치가 자신에게 맞는 실행 파일을 자동 선택합니다. 실제 원형 화면 해상도에 맞춰 화면과 터치 좌표를 자동 조정하고 9개 지역 팩을 포함하는 독립형 Wear OS 앱입니다. 성장·시계와 배터리 표시는 워치의 날짜·시간 및 실제 잔량을 따릅니다. LAN 대전과 세이브 전송은 워치 Wi-Fi를 휴대전화와 같은 공유기 또는 실기의 `TamaPoke-XXXX` 방에 연결해 사용합니다.
...
The good-farewell system is replaced with two menu egg claims per local calendar day. Your current Pokemon is preserved in the party first, then the box, without level/age/evolution conditions. An existing egg or full storage denies the claim without charging. Unused claims do not carry over. Startup eggs and automatic eggs after neglect/runaway remain separate. Care-streak/Dex bonuses, shared energy and existing saves are preserved.
