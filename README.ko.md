...
**3.7.3 과도한 확대 수정:** 홈 기준 크기를 줄이고 원본 그림 대비 최대 4배까지만 확대합니다. 기본·폼·이로치 애니메이션에 같은 상한을 적용하며 파티·박스 아이콘과 저장·밸런스는 유지합니다. [수정 내용](docs/RELEASE-3.7.3.ko.md).

**3.7.2 그림 크기·유대 버그 패치:** 작은 종과 큰 종의 차이를 살리며 투명 여백을 보정하고, 파티·박스의 작은 폼 그림과 이름 겹침을 수정했습니다.  날짜 변경 후 교체한 동료의 유대 한도 잔류, 쓰다듬기 저장 누락과 새날 첫 행동 누락을 수정했습니다. 행동 유대는 개체별 하루 20까지이며 첫 돌봄 +4는 별도입니다. [수정 내용](docs/RELEASE-3.7.2.ko.md).

**3.7.1 워치 통신 버그 수정:** 워치가 통신할 Wi-Fi를 확보하고 수신 필터·재연결 처리를 보완했습니다. 폰과 워치 간 통신 실패가 수정 테스트판에서 해결됐다는 사용자 확인 후 정식 배포합니다. 통신 화면의 IP·TX·RX·E로 연결 상태를 확인할 수 있습니다. 저장 형식과 밸런스는 그대로입니다.

**3.7.0 공용 활력:** 활력을 공용으로 통합합니다. 포획한 포켓몬이나 파티·박스 동료를 데려와도 활력이 바뀌지 않으며 작별 후 새 알을 받아도 유지됩니다. 탐색 소모량은 22, 수면 회복은 기존 8/분입니다. 이전 세이브의 현재 활력을 이어받습니다.

**3.6.1 진화 계열 잠금 버그 수정:** 그림 없는 모습으로 진화하는 하위 15종을 탐색에서도 제외하고 알·도감 보너스와 기준을 통일했습니다. 수집 가능 도감은 967종, 50% 보너스는 484종 이상입니다. 도감 자물쇠로 잠긴 종을 표시하며 기존 보유 개체·도감 기록은 보존합니다. 잠긴 기존 개체의 진화는 차단합니다.

**3.6.0 도감 포획 보너스:** 수집 가능한 도감 100종마다 탐색 포획식의 볼 배율이 0.2씩 증가합니다. 포획률 3의 2.5% 보정에도 같은 배율이 적용되어 900종부터 7%입니다. 기존 도감 기록을 바로 반영하며 중복·이로치·폼은 중복 집계하지 않습니다. 조우·이로치·알 보너스와 저장 형식은 유지합니다. 단계별 다섯 종의 확률표는 플레이 가이드 29쪽입니다.

**Windows 워치 설치 도우미 추가:** 정식 3.7.3 APK와 7쪽 설명서를 동봉한 [한국어 설치 도우미 시험판](docs/WATCH-INSTALLER-3.7.3.ko.md)을 릴리스에서 받을 수 있습니다. 압축을 모두 풀고 실행하면 명령어 없이 등록·연결·설치할 수 있습니다. 실제 워치 설치는 아직 미검증이며 기존 수동 설치 PDF도 유지합니다.

**3.5.0: 한 마리 전송과 1:1 교환.** 통신 메뉴 → 포켓몬 전송 / 교환에서 파티·박스 개체를 선택합니다. 양쪽에서 상대·코드·포켓몬을 확인하고 확정하세요. 확정 이후 중단되면 같은 상대와 재연결해 완료합니다. 거래 중에는 전체 백업/복원과 보관 변경을 막으므로 먼저 백업하세요. [사용법과 주의사항](docs/RELEASE-3.5.0.ko.md), [복구 설계](docs/INDIVIDUAL-TRANSFER.ko.md). 플레이 가이드 26~27쪽에도 설명합니다.

3.4.0부터 마지막 순서의 포켓몬이 쓰러져도 앞쪽에 살아 있는 동료가 있으면 배틀을 계속하도록 수정했습니다. 탐색 이로치는 1%이며, 이로치 알은 도감 등록 여부와 무관하게 종을 고릅니다. 일반 알의 미획득 우선과 도감·연속 돌봄 알 보너스는 유지합니다. 연속 10일 또는 도감 50% 단독 목표는 이로치 10%·전설 14%, 두 목표를 모두 달성하면 최대 15%·21%입니다. 기존 세이브를 유지하며 앱을 삭제하지 말고 업데이트하세요.

**정식 3.7.3** · 애니메이션 지원 폼 176개, 폼별 타입·능력치·기술, 가라르·팔데아 체육관, 자유 교체, 300칸 박스와 연속 돌봄 보너스를 제공합니다. [릴리스 안내](docs/RELEASE-3.7.3.ko.md) · [최신 플레이 가이드](web/guides/TamaPoke-3.7.3-Play-Guide-KO.pdf)

**업데이트 전 세이브를 백업하세요.** 저장 v7은 이전 v1~v6을 읽지만 구버전으로 되돌릴 수 없습니다. 근거리 연결은 양쪽을 3.7.3로 맞추세요. **ESP는 300칸 보관 기록용 microSD가 필수**이며 기존 SD와 내부 저장을 함께 유지해야 합니다. 앱에는 9개 지방과 폼 그림 팩이 포함됩니다.
...
![펌웨어](https://img.shields.io/badge/펌웨어-3.7.3-8A2BE2)
...
현재 정식 버전은 **3.7.3**입니다. 지방과 탐색 방식을 골라 야생 포켓몬과 싸우고 승리 뒤 포획하는 탐색 시스템을 추가했습니다. 조우 1회당 공용 활력 22를 사용하며 일반은 현재 포켓몬 레벨 ±5, 랜덤은 레벨 1~100에서 상대가 정해집니다. 파티는 현재 키우는 포켓몬 1마리와 보관 파티 5마리를 합쳐 전투 후보가 최대 6마리이며, 300칸/50페이지 박스를 사용하며 이전 세이브를 자동 변환합니다. Android·Wear OS의 배터리 표시는 실제 기기 잔량과 충전 상태를 따릅니다.

현재 3.7.3 한국어 화면을 넣은 **29쪽 초보자용 플레이 설명서**는 [설치 페이지의 설명서 다운로드](https://loaram.github.io/TamaPoke_ko/#play-guide)에서 PDF 또는 페이지별 이미지로 받을 수 있습니다. 탐색의 조우 레벨·종류 확률과 포획 확률은 11~12쪽, 기기 조합별 근거리 배틀은 13~15쪽, 세이브 이전은 16~18쪽, 폼·연속 돌봄·도감 보너스·자유 교체는 19~22쪽, 박스 정렬은 23쪽, 동료 성장·개체값은 24쪽, 탐색 결과 확인창은 25쪽에서 설명합니다. 데려오기 제한과 저장 복구 안내는 28쪽입니다.

박스에서 포켓몬을 누르면 항상 상세창이 열리며 **놓아주기 → 예**로 바로 정리할 수 있습니다. 교체 대상을 먼저 골랐더라도 상세창의 **교체** 버튼을 눌러 확정합니다. 파티/박스 놓아주기는 하루 작별 3회에 포함되지 않습니다.

박스 아래 **정렬 → 가나다순 / 번호순 / 레벨순 → 예**로 300칸 전체를 한 번 정렬합니다. 공식 한국어 종 이름(별명 제외), 도감 번호 오름차순, 레벨 내림차순이며 빈칸은 뒤로 모읍니다. 적용 후 첫 페이지로 돌아가고 이전 교체 선택은 취소됩니다. 새 포획·보관은 자동 정렬하지 않습니다.

포획·좋은 작별 동료도 **데려오기**로 홈에 오면 20분마다 1레벨씩, 최대 100레벨까지 성장하고 조건을 갖추면 일반 진화합니다. 보관 중에는 멈추며 기존 도망 방지·재작별 제외는 유지합니다. 파티/박스 상세창의 **공격 99(31)**에서 괄호 안 숫자는 **개체값(0~31)**입니다.

## 한국어 게임 화면

3.7.3의 도감은 `도감 1/1025(967)`처럼 등록 종수 / 전체 번호 수(현재 수집 가능한 종수)를 표시합니다. 지역별 도감에도 같은 기준을 적용하며, 번호·세이브·게임 밸런스는 유지합니다.

버전은 `a.b.c`를 사용합니다. 주요 기능·대규모 변경은 a, 편의 기능·기능 수정·밸런스 패치는 b, 단순 텍스트·간단한 패치는 c를 올리고 상위 번호가 바뀌면 하위 번호는 0으로 초기화합니다.
...
지원 기기는 **Waveshare ESP32-S3-Touch-AMOLED-1.75**입니다. PC용 Chrome 또는 Edge에서 이 저장소의 GitHub Pages 설치 페이지를 이용합니다. 로컬 설치 페이지 실행은 아래 개발자 안내를 참고하세요.
...
5. 관동·성도·호연·신오·하나·칼로스·알로라·가라르·팔데아 중 원하는 팩과 폼체인지 팩을 설치하거나 **지방·폼 전체 설치**를 선택합니다.
...
2. 양쪽의 **통신 메뉴** 화면에서 원본 기기는 **세이브 보내기**, 대상 기기는 **세이브 받기**를 누릅니다.
3. 양쪽의 6자리 확인 코드가 같은지 확인하고 전송이 끝날 때까지 화면을 유지합니다.
4. 대상 기기에 **받기 완료**가 나오면 **세이브 적용 → 예**를 누릅니다. ESP32는 재시작합니다. Android·워치는 앱이 닫히면 직접 다시 열어 받은 세이브를 확인하세요.
...
- 시작할 때 공용 활력 22를 사용합니다. 일반은 현재 키우는 포켓몬의 레벨 ±5, 랜덤은 레벨 1~100에서 같은 확률로 상대 레벨을 정합니다.
...
- UI 문자열 194개, 포켓몬 1025종 이름, 기술표 717개(빈 기술 포함 718항목), 18개 타입, 지방·트레이너·장소 표시.
- 메뉴, 도감, 상태, 성장, 기술 선택, 배틀, 파티, 박스, 체육관, 통신 메뉴·세이브 전송, 언어 설정.
- 기존 6개 언어와 1.1.6 이하 저장 데이터는 자동 변환해 유지합니다. 3.7.3의 무선 규격은 프로토콜 5이며 세이브 전송에는 현재 포켓몬, 파티 5마리, 300칸 박스, 도감, 배지와 설정이 포함됩니다.
...
결과는 `build/android/TamaPoke-3.7.3-Android-Full-debug.apk`입니다. Android 기기용 `arm64-v8a`와 에뮬레이터용 `x86_64`, 9개 지역 스프라이트 팩을 모두 포함합니다. 성장·시계와 배터리 표시는 Android 기기 설정과 실제 잔량을 직접 따릅니다. 프로젝트별 디버그 키는 무시되는 `build/android/debug.keystore`에 생성됩니다. 다른 PC의 디버그 키로 서명한 구버전 APK가 이미 설치돼 있으면 서명이 달라 덮어쓸 수 없습니다. 기존 앱을 삭제하면 그 앱의 Android 저장 데이터도 함께 지워지므로, 저장이 중요하면 삭제하지 말고 원래 키스토어로 새 APK를 서명해야 합니다. 이번 키로 후속 APK를 계속 업데이트하려면 `debug.keystore`도 별도로 안전하게 보관해야 합니다.
...
```powershell
.\.venv\Scripts\python.exe tools\prepare_release_guides.py --out build\release\3.7.3
```

현재 버전의 `web/guides/TamaPoke-<버전>-Play-Guide-KO.pdf`가 없으면 도구와 GitHub 검사가 실패하므로, 새 버전에서는 플레이 가이드를 먼저 갱신해야 합니다.

앱끼리 대전할 때는 두 기기를 같은 Wi-Fi에 연결한 뒤 양쪽에서 통신 메뉴과 팀을 선택합니다. 앱과 실기가 대전할 때는 실기에서 먼저 통신 메뉴 팀을 선택하고 화면에 표시되는 `TamaPoke-XXXX` Wi-Fi를 Android 설정에서 선택합니다. 암호는 `tamapoke`입니다. 인터넷 연결이 없다는 Android 안내가 나오면 연결 유지를 선택한 뒤 앱으로 돌아와 통신 메뉴 팀을 선택합니다. 방 만들기/참가하기를 양쪽이 똑같이 눌러도 고유 번호로 역할을 자동 조정합니다.
