...
**Windows 워치 설치 도우미 추가:** 정식 3.9.1 APK와 7쪽 설명서를 동봉한 [한국어 설치 도우미 시험판](docs/WATCH-INSTALLER-3.9.1.ko.md)을 릴리스에서 받을 수 있습니다. 압축을 모두 풀고 실행하면 명령어 없이 등록·연결·설치할 수 있습니다. 실제 워치 설치는 아직 미검증이며 기존 수동 설치 PDF도 유지합니다.
...
**정식 3.9.1** · 애니메이션 지원 폼 176개, 폼별 타입·능력치·기술, 가라르·팔데아 체육관, 자유 교체, 300칸 박스와 연속 돌봄 보너스를 제공합니다. [릴리스 안내](docs/RELEASE-3.9.1.ko.md) · [최신 플레이 가이드](web/guides/TamaPoke-3.9.1-Play-Guide-KO.pdf)

**업데이트 전 세이브를 백업하세요.** 저장 v7은 이전 v1~v6을 읽지만 구버전으로 되돌릴 수 없습니다. 근거리 연결은 양쪽을 3.9.1로 맞추세요. **ESP는 300칸 보관 기록용 microSD가 필수**이며 기존 SD와 내부 저장을 함께 유지해야 합니다. 앱에는 9개 지방과 폼 그림 팩이 포함됩니다.
...
![펌웨어](https://img.shields.io/badge/펌웨어-3.9.1-8A2BE2)
...
현재 정식 버전은 **3.9.1**입니다. 지방과 탐색 방식을 골라 야생 포켓몬과 싸우고 승리 뒤 포획하는 탐색 시스템을 추가했습니다. 조우 1회당 공용 활력 22를 사용하며 일반은 현재 포켓몬 레벨 ±5, 랜덤은 레벨 1~100에서 상대가 정해집니다. 파티는 현재 키우는 포켓몬 1마리와 보관 파티 5마리를 합쳐 전투 후보가 최대 6마리이며, 300칸/50페이지 박스를 사용하며 이전 세이브를 자동 변환합니다. Android·Wear OS의 배터리 표시는 실제 기기 잔량과 충전 상태를 따릅니다.

현재 3.9.1 한국어 화면을 넣은 **31쪽 초보자용 플레이 설명서**는 [설치 페이지의 설명서 다운로드](https://loaram.github.io/TamaPoke_ko/#play-guide)에서 PDF 또는 페이지별 이미지로 받을 수 있습니다. 탐색의 조우 레벨·종류 확률과 포획 확률은 11~12쪽, 기기 조합별 근거리 배틀은 13~15쪽, 세이브 이전은 16~18쪽, 폼·연속 돌봄·도감 보너스·자유 교체는 19~22쪽, 박스 정렬은 23쪽, 동료 성장·개체값은 24쪽, 탐색 결과 확인창은 25쪽에서 설명합니다. 데려오기 제한과 저장 복구 안내는 28쪽입니다.
...
3.9.1의 도감은 `도감 1/1025(967)`처럼 등록 종수 / 전체 번호 수(현재 수집 가능한 종수)를 표시합니다. 지역별 도감에도 같은 기준을 적용하며, 번호·세이브·게임 밸런스는 유지합니다.
...
- 기존 6개 언어와 1.1.6 이하 저장 데이터는 자동 변환해 유지합니다. 3.9.1의 무선 규격은 프로토콜 5이며 세이브 전송에는 현재 포켓몬, 파티 5마리, 300칸 박스, 도감, 배지와 설정이 포함됩니다.
...
```powershell
.\.venv\Scripts\python.exe tools\prepare_release_guides.py --out build\release\3.9.1
```
