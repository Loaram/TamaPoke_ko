...
![펌웨어](https://img.shields.io/badge/펌웨어-ko.1.1.4-8A2BE2)
...
현재 정식 버전은 **ko.1.1.4**입니다. 성장 속도 20분당 1레벨에 맞춰 최종 진화 포켓몬의 작별 제안 시점을 2일에서 **1일(24시간, 레벨 73)**로 줄였습니다. Android·Wear OS의 성장 시간과 시계 화면은 휴대전화·워치의 날짜 및 시간 설정을 직접 따르며, 시각을 뒤로 조정해도 잘못된 대량 레벨업이 발생하지 않습니다. Android·Wear OS·ESP32 사이의 세이브 전송, 전국도감 1025종, 수면 중 활력 분당 15 회복, 조기 돌봄 종료 패널티 제거와 LAN 대전도 그대로 포함합니다.
...
- 기존 6개 언어와 저장 데이터 형식을 유지합니다. ko.1.1.4의 무선 규격은 프로토콜 3이며 ko.1.1.2~ko.1.1.3과 호환됩니다. 프로토콜 2 이하 버전과의 LAN 연결은 거부됩니다.
...
결과는 `build/android/TamaPoke-ko.1.1.4-Android-Full-debug.apk`입니다. 실제 기기용 `arm64-v8a`와 에뮬레이터용 `x86_64`, 9개 지역 스프라이트 팩을 모두 포함합니다. ko.1.1.3부터 성장과 시계 화면은 Android 설정의 날짜 및 시간을 직접 따르며 앱 안에서 별도 시각 오프셋을 만들지 않습니다. 프로젝트별 디버그 키는 무시되는 `build/android/debug.keystore`에 생성됩니다. 다른 PC의 디버그 키로 서명한 구버전 APK가 이미 설치돼 있으면 서명이 달라 덮어쓸 수 없습니다. 기존 앱을 삭제하면 그 앱의 Android 저장 데이터도 함께 지워지므로, 저장이 중요하면 삭제하지 말고 원래 키스토어로 새 APK를 서명해야 합니다. 이번 키로 후속 APK를 계속 업데이트하려면 `debug.keystore`도 별도로 안전하게 보관해야 합니다.
