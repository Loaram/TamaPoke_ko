...
int main(){
  ...
  ck(p.eggBonusDays()==0 && p.eggShinyWeight()==450 && p.eggLegendWeight()==27,"unknown clock gives base 1/48 shiny and 3% legendary");
  for(int d=1;d<=11;d++){
    day(p,100+d);p.feed();
    int step=d>10?9:d-1;
    ck(p.streak==d && p.eggShinyWeight()==450+190*step && p.eggLegendWeight()==27+11*step,"daily care increases both weights, capped at day ten");
    p.feed();p.clean();p.caress();
    ck(p.streak==d,"multiple care actions do not count as multiple days");
  }
  ck(p.eggShinyWeight()*10==21600 && p.eggLegendWeight()*100==14*900,"day ten matches old actual maxima: 10% shiny / 14% legendary");
  {
    ...
    for(int careDay: {1,10}){
      ...
      ck(careDay==1 ? (shiny>65 && shiny<145) : (shiny>400 && shiny<600),"actual egg creation and hatching use daily shiny odds");
    }
  }
  for(int end: {CER_NONE,CER_RELEASE,CER_FAREWELL}){
    p.lastEnd=end;p.bond=0;
    ck(p.eggShinyWeight()==2160 && p.eggLegendWeight()==126,"farewell classification and individual bond no longer set these bonuses");
  }
  int legends=0;
  ...
  ck(p.eggLegendWeight()==0 && p.eggShinyWeight()==2160,"runaway retains common-only restriction, not a shiny bonus penalty");
  p.lastEnd=CER_NONE;
  ...
  day(p,113);ck(p.eggBonusDays()==0 && p.eggShinyWeight()==450 && p.eggLegendWeight()==27,"missing a full care day removes bonuses without waiting for another action");
