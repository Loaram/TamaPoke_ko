...
int main(){
  nvs().clear();Pet p;ready(p);p.startRetire();
  ...
  nvs().clear();Pet crash;ready(crash);crash.startRetire();auto old=nvs();finish(crash);
  ...
  nvs().clear();Pet failed;ready(failed);failed.startRetire();nvsFailKey()="endWait";finish(failed);
  ...
  nvs().clear();Pet early;ready(early);early.ageMinutes=10;early.startRetire();Pet earlyReload;earlyReload.begin();finish(earlyReload);
