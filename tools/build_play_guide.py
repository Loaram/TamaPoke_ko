...
OUT = ROOT / "output" / "pdf" / "TamaPoke-3.1.1-Play-Guide-KO.pdf"
VERSION = "3.1.1"
PAGE_TOTAL = 24
...
story.extend(
    [
        ...
        p("알의 이로치·전설 보너스는 <b>플레이어의 연속 돌봄 10일</b>에 상한에 도달합니다. 좋은 작별·일반 작별·현재 포켓몬 보내기는 합쳐 하루 3회이며 자정에 초기화됩니다. 자세한 확률은 21쪽에서 확인하세요.", BODY),
    ]
)
page_break(story)
...
story.extend(
    [
        ...
        p("릴리스: https://github.com/Loaram/TamaPoke_ko/releases/tag/3.1.1", SMALL),
...
page_break(story)
story.extend(page_heading("20 매일 돌봄", "연속 10일 알 보너스와 작별 3회", "포켓몬을 교체해도 플레이어의 연속 돌봄 기록은 유지됩니다."))
story.append(info_table([
    ["연속 돌봄", "이로치 알", "전설 등급 추첨"],
    ["기록 없음 / 1일차", "약 2.08% (1/48)", "3.00%"],
    ["2일차", "약 2.96%", "약 4.22%"], ["3일차", "약 3.84%", "약 5.44%"],
    ["4일차", "약 4.72%", "약 6.67%"], ["5일차", "약 5.60%", "약 7.89%"],
    ["6일차", "약 6.48%", "약 9.11%"], ["7일차", "약 7.36%", "약 10.33%"],
    ["8일차", "약 8.24%", "약 11.56%"], ["9일차", "약 9.12%", "약 12.78%"],
    ["10일차 이후", "10.00%", "14.00%"],
], [56*mm,59*mm,59*mm]))
story.extend([
    Spacer(1, 4*mm),
    bullet("먹이·목욕·쓰다듬기·훈련 등 실제 돌봄을 한 날짜를 하루 한 번 기록합니다. 앱만 켜거나 알만 두드리는 것은 제외합니다. 다시 데려온 동료를 돌봐도 됩니다."),
    bullet("어제까지의 보너스는 오늘 첫 돌봄 전에도 유지됩니다. 돌보지 않은 날이 하루 통째로 지나면 기본 확률로 돌아가고 다음 돌봄부터 1일차입니다."),
    bullet("전설은 <b>도감 25종 등록 후</b>부터 추첨합니다. 해당 지역에 후보가 없으면 낮은 등급으로 대체합니다. 방치로 도망간 뒤 다음 알은 일반 등급만 나옵니다."),
    bullet("이미 생긴 알은 다시 추첨하지 않습니다. 탐색의 조우·포획 확률과 이 표는 별개입니다."),
    p("<b>좋은 작별·일반 작별·현재 개체 보내기를 합쳐 하루 3회</b>입니다. 확인 후 시작할 때 차감되며 기기의 현지 자정에 초기화됩니다. 메뉴의 2/3은 오늘 남은 횟수입니다. 교체나 보관 개체 정리는 차감하지 않습니다.", SAFE),
])
