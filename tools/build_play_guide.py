...
OUT = ROOT / "output" / "pdf" / "TamaPoke-3.9.3-Play-Guide-KO.pdf"
VERSION = "3.9.3"
PAGE_TOTAL = 31
...
story.extend(page_heading("04 육성", "성장·수면·훈련·알 받기", "돌봄과 훈련이 진화, 능력치와 다음 알에 미치는 영향을 확인하세요."))
story.append(
    info_table(
        [
            ["항목", "규칙", "플레이 팁"],
            ...
            ["알 받기", "메뉴에서 하루 최대 2회 직접 수령", "자정 초기화 · 이월 없음"],
            ["수면 활력", "분당 +8", "실시간과 전원 꺼짐 시간 모두 적용"],
            ["알 수령 시 보관", "현재 포켓몬은 파티 우선 보관", "파티가 가득 차면 박스로 이동"],
...
story.extend(
    [
        ...
        p("하루 두 번 알 받기와 알 보너스", H2),
        p("이름을 눌러 메뉴의 <b>알 받기 → 알 받기</b>를 선택하세요. 현재 포켓몬을 파티 우선, 가득 차면 박스에 보관하고 홈에 새 알을 놓습니다. 레벨·성장 시간·진화 단계 조건은 없습니다.", SAFE),
        p("알 보너스는 <b>연속 돌봄 10일과 수집 가능 도감 50%</b>를 모두 달성하면 최대 이로치 15%·전설 21%입니다. 메뉴 수령은 하루 2회이며 자정에 초기화됩니다. 이미 알이 있거나 보관할 빈칸이 없으면 받을 수 없고 횟수도 차감하지 않습니다. 자세한 확률은 21쪽에서 확인하세요.", BODY),
    ]
)
page_break(story)
...
story.extend(
    [
        ...
        bullet("도감 1/1025(967)는 등록 1종 / 전체 번호 1025종 / 현재 수집 가능 967종입니다. 괄호는 알·진화·탐색으로 모을 수 있는 종수이며, 지역별로도 표시합니다."),
        bullet("그림 미지원 43종과 그 모습으로 진화할 수 있는 하위 15종은 알·탐색에서 제외됩니다. 도감 자물쇠는 수집 잠금입니다. 기존 보유·등록은 보존하지만 잠긴 개체는 진화하지 않습니다."),
...
story.extend(
    [
        ...
        bullet("박스의 포켓몬을 누르면 상세창이 열립니다. <b>놓아주기 → 예</b>로 바로 정리하며 파티를 거칠 필요가 없습니다. 파티/박스 놓아주기는 알 수령 횟수와 별개이며 횟수를 돌려주지 않습니다."),
        ...
        bullet("알 수령 시 현재 포켓몬과 새 포획은 파티 우선, 파티가 가득 차면 박스로 들어갑니다. 둘 다 가득 차면 알 수령·탐색을 시작할 수 없습니다."),
        bullet("파티/박스 상세창의 <b>데려오기</b>로 현재 키우는 포켓몬과 자유롭게 맞교환합니다. 성장 개체는 보관 중 시간이 멈추고 다시 데려오면 이어집니다."),
        bullet("전체 세이브 이전에는 현재 포켓몬, 300칸 박스, 폼·기술, 도감·배지, 연속일수·알 수령 횟수가 함께 들어갑니다."),
...
story.extend(
    [
        ...
        Spacer(1, 4 * mm),
        p("탐색 시작 조건", H2),
        p("알·수면·이전 종료 연출 중이 아니며 활력이 22 이상이어야 합니다. 파티 또는 박스에 빈 칸도 필요합니다. 활력 100에서는 회복 없이 최대 4회 탐색하며 12가 남습니다.", CALLOUT),
...
story.extend(
    [
        ...
        Spacer(1, 4 * mm),
        p("먼저 위 종류를 뽑고, 선택한 지방의 수집 가능 후보 중 하나를 같은 확률로 고릅니다. 그림 없는 종과 그 모습으로 진화할 수 있는 하위 종은 알·탐색에서 제외합니다. 후보 수에 따라 특정 종의 조우 확률은 달라집니다.", BODY),
...
story.extend(
    [
        ...
        p("릴리스: https://github.com/Loaram/TamaPoke_ko/releases/tag/3.9.3", SMALL),
...
story.extend([
    ...
    p("폼 해금은 알 수령과 별개입니다. 새 알은 레벨·진화 단계와 관계없이 메뉴에서 하루 두 번 받을 수 있습니다.", SAFE),
])
...
story.append(screenshot_pair(ROOT / "docs/qa/3.8.0/egg-bonus-dex-only.png", ROOT / "docs/qa/3.8.0/egg-bonus-max.png", 48*mm))
...
story.extend([
    Spacer(1, 3*mm),
    bullet("도감률은 <b>등록한 수집 가능 종 / 전체 수집 가능 종</b>입니다. 현재 967종 중 <b>484종(50% 이상)</b>이 목표입니다. 그림 미지원 43종과 하위 진화 계열 15종은 분자·분모에서 제외하며 폼·이로치·중복은 한 종으로 셉니다."),
    ...
    p("<b>메뉴의 알 받기는 하루 2회</b>입니다. 현지 자정 초기화, 미사용분 이월 없음. 첫 시작 알과 <b>방치로 도망간 뒤 자동 생성되는 알은 별개</b>입니다. 교체·보관 개체 정리는 횟수를 바꾸지 않습니다.", SAFE),
])

page_break(story)
story.extend(page_heading("21 보관과 안전", "자유 교체와 세이브를 지키는 방법", "현재 포켓몬과 보관 개체의 교환은 알이 아닐 때도 가능합니다."))
story.append(screenshot_pair(ROOT / "docs/qa/3.8.0/daily-rewards-menu.png", ROOT / "docs/qa/3.8.0/daily-rewards-confirm.png", 42*mm))
story.extend([
    step_table(["파티나 박스에서 데려올 포켓몬을 누릅니다.", "상세창의 <b>데려오기</b>를 누르면 현재 포켓몬은 그 보관 칸으로 들어오고 선택한 포켓몬이 홈으로 옵니다.", "성장 분·폼·기술·개체값·훈련·돌봄·유대는 보존되며 보관 중에는 멈춥니다. 단, <b>활력은 공용</b>이므로 교체 전 수치를 그대로 이어받습니다.", "포획하거나 보관한 동료도 홈으로 데려오면 현재 레벨에서 성장하고 조건을 만족하면 진화합니다. 보관 중에는 멈춥니다."]),
    Spacer(1, 5*mm),
    p("ESP32의 SD 카드", H2),
    ...
    bullet("알 수령 횟수와 미완료 수령 기록도 저장됩니다. 저장 확인 안내가 뜨면 초기화하지 말고 저장 공간·SD를 확인한 뒤 다시 확인을 누르세요. 중단된 수령은 같은 알로 이어지고 중복 차감하지 않습니다."),
...
page_break(story)
story.extend(page_heading("23 동료 육성·개체값", "다시 데려온 동료도 함께 성장", "포획하거나 보관한 포켓몬도 키우는 자리에서 성장할 수 있습니다."))
story.append(screenshot_pair(ROOT / "docs/qa/3.1.0/companion-party-ivs.png", ROOT / "docs/qa/3.1.0/companion-box-ivs.png"))
story.extend([
    Spacer(1, 4*mm),
    step_table(["파티나 박스에서 포켓몬을 누르고 <b>데려오기</b>를 선택합니다. 현재 포켓몬과 자리를 맞바꿉니다.", "홈에서 함께 지내면 현재 레벨부터 <b>20분마다 1레벨</b>씩 성장합니다. 60레벨은 20분 후 61레벨, 73레벨 동료는 20분 후 74레벨이 됩니다.", "일반 진화 조건을 만족하면 진화할 수 있습니다. 필요한 레벨과 돌봄 상태를 갖추고 진화를 선택하세요. 최대 레벨은 100입니다."]),
    Spacer(1, 3*mm),
    bullet("파티·박스 안에서는 성장하지 않습니다. 다시 보관했다 데려와도 진행 중이던 성장 분·기술·폼·개체값·훈련 기록이 이어집니다."),
    bullet("포획·보관 동료는 도망 방지 보호를 유지합니다. 이 동료를 키우는 중에도 새 알을 받을 수 있습니다. 포켓몬을 정리하려면 파티·박스에서 놓아주세요."),
    p("능력치 옆 괄호 읽기", H2),
    ...
    Spacer(1, 3*mm),
    p("기존 세이브의 모든 동료도 그대로 사용할 수 있습니다. 개체값을 확인하는 것만으로 포켓몬이나 저장 데이터가 바뀌지 않습니다.", SAFE),
])
...
story.extend([
    ...
    bullet("전송·교환은 알 수령 횟수를 사용하지 않습니다. 받은 포켓몬은 홈에서 성장·진화할 수 있고 기존 도망 방지 등 개별 상태는 그대로 유지합니다."),
...
story.extend([
    ...
    bullet("전체 지방의 수집 가능한 종을 합산합니다. 같은 종의 중복·이로치·폼은 한 종으로 세며, 그림 미지원 종은 제외합니다. 현재 수집 가능 종수는 967종입니다."),
