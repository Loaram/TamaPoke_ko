"""Verify the exact 3.9.2 public bundle and record reproducible evidence."""
import hashlib,json,re,zipfile
from pathlib import Path
from pypdf import PdfReader
from capture_guide_data import capture_rows,EXAMPLES
R=Path(__file__).resolve().parents[1];V='3.9.2';A=R/'build/release'/V;Q=R/'docs/qa'/V
Q.mkdir(parents=True,exist_ok=True)
def sha(p):
    with p.open('rb') as f:return hashlib.file_digest(f,'sha256').hexdigest()
markers={'android':'versionCode 3056','wear':'versionCode 3057','esp':'Published verified Korean components',
         'emu':'tamapoke-emu.exe','watch-installer':'30 checks passed.',
         'runtime':'PASS: 67 runtime suites','swap-final':'PASS: shared_energy,swap_recovery,active_swap'}
for name,marker in markers.items():
    s=(R/f'build/release-392-{name}.log').read_text(encoding='utf8',errors='replace')
...
report=dict(version=V,save_version=7,record_bytes=72,android_version_code=3056,wear_version_code=3057,
  local_runtime_suites=67,final_swap_regression=True,sd_large_allocation_fault_reproduced=True,
  sd_streaming_verification_bytes=256,installer_checks=30,play_guide_pages=31,watch_guide_pages=7,
  final_runtime_ci_required=True,physical_swap_retest=False,physical_root_cause_confirmed=False,
