"""Verify the exact public 3.7.1 bundle and record local evidence."""
import hashlib,json,re,zipfile,sys
from pathlib import Path
from pypdf import PdfReader
from capture_guide_data import probability, capture_rows, EXAMPLES
R=Path(__file__).resolve().parents[1];V='3.7.1';A=R/'build/release'/V;Q=R/'docs/qa'/V
Q.mkdir(parents=True,exist_ok=True)
def sha(p):
    with p.open('rb') as f:return hashlib.file_digest(f,'sha256').hexdigest()
markers={'android':'versionCode 3042','wear':'versionCode 3043','esp':'Published verified Korean components',
         'emu':'tamapoke-emu.exe','watch-installer':'30 checks passed.'}
for name,marker in markers.items():
    s=(R/f'build/release-371-{name}.log').read_text(encoding='utf8',errors='replace')
...
runtime=(R/'build/release-371-runtime.log').read_text(encoding='utf8',errors='replace')
assert 'PASS: 62 runtime suites including Android UDP transport, shared energy and save recovery' in runtime
assert 'TamaPoke fw v3.7.1' in runtime
assert '202 party/box swaps' in runtime and 'ten interrupted party/box commits' in runtime
assert 'MATRIX rate_count_cases=247808' in runtime
assert len(re.findall(r'^SIM dex=',runtime,re.M))==50
sd=(R/'build/release-371-sd-store.log').read_text(encoding='utf8',errors='replace')
assert len(re.findall(r'^PASS ',sd,re.M))==10
for name,data in [('runtime.log',runtime),('sd-store.log',sd)]:
    assert not re.search(r'^FAIL\b|Traceback|AssertionError',data,re.M)
    (Q/name).write_text('\n'.join(line.rstrip() for line in data.splitlines())+'\n',encoding='utf8')
report=dict(version=V,runtime_suite_total=62,runtime_environment='Windows local',
    shared_energy=True,exploration_cost=22,stored_energy_swap_cases=202,swap_write_faults=10,
    capture_loop='party and box success/bring/reload loops stop after four attempts from 100 energy',
    rate_count_cases=247808,capture_simulation_draws=5000000,sd_failure_checks=10,installer_checks=30,
    device_tested='User reported phone-watch fix successful; other models and ESP hardware unverified',save_version=7,record_bytes=72,battle_protocol=5,trade_protocol=1,
    android_version_code=3042,wear_version_code=3043,
...
print('PASS: 62 runtime suites, shared energy, capture and SD simulations, nine exact public assets')
java=(R/'build/release-371-java.log').read_text(encoding='utf8',errors='replace')
assert 'PASS: 20 actual Activity callback checks' in java
(Q/'java.log').write_text(java,encoding='utf8')
assert len(re.findall(r'^PASS native UDP preserves full save byte for byte',runtime,re.M))==4
print('PASS: 20 Java callbacks and bidirectional native UDP loss scenarios')
