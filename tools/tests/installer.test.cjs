...
(async()=>{
  ...
  for(const version of ['2.0.0','ko.1.0.1','ko.1.1.1','3.11-ko.1','3.11','ko.bad']) {
    ...
    await new Promise(setImmediate);
    assert.equal(get('firmware').disabled,!['2.0.0','ko.1.0.1','ko.1.1.1','3.11-ko.1'].includes(version),version);
  }
