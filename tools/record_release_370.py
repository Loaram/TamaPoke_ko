"""Verify the exact public 3.7.0 bundle and record local evidence."""
import hashlib,json,re,zipfile,sys
from pathlib import Path
from pypdf import PdfReader
from capture_guide_data import probability, capture_rows, EXAMPLES
R=Path(__file__).resolve().parents[1];V='3.7.0';A=R/'build/release'/V;Q=R/'docs/qa'/V
Q.mkdir(parents=True,exist_ok=True)
def sha(p):
    with p.open('rb') as f:return hashlib.file_digest(f,'sha256').hexdigest()
markers={'android':'versionCode 3040','wear':'versionCode 3041','esp':'Published verified Korean components',
         'emu':'tamapoke-emu.exe','watch-installer':'30 checks passed.'}
for name,marker in markers.items():
    s=(R/f'build/release-370-{name}.log').read_text(encoding='utf8',errors='replace')
...
runtime=(R/'build/release-370-runtime.log').read_text(encoding='utf8',errors='replace')
assert 'PASS: 61 runtime suites including shared energy, collection locks, capture bonus and save recovery' in runtime
assert 'TamaPoke fw v3.7.0' in runtime
...
sd=(R/'build/release-370-sd-store.log').read_text(encoding='utf8',errors='replace')
...
report=dict(version=V,runtime_suite_total=61,runtime_environment='Windows local',
    ...
    device_tested=False,save_version=7,record_bytes=72,battle_protocol=5,trade_protocol=1,
    android_version_code=3040,wear_version_code=3041,
...
print('PASS: 61 runtime suites, shared energy, capture and SD simulations, nine exact public assets')
