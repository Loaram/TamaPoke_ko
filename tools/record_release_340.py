"""Verify and archive the 3.4.0 public artifact set, without publishing it."""
import argparse,hashlib,json,re,shutil,subprocess,sys,zipfile
from pathlib import Path
from pypdf import PdfReader
R=Path(__file__).resolve().parents[1];Q=R/'docs/qa/3.4.0';A=R/'build/release/3.4.0'
...
logs={'runtime':'53 runtime suites','focused':'PASS: battle_reserves,lan,wild_result,shiny_eggs','sd-store':'PASS factory reset','android':'versionCode 3026',
      'wear':'versionCode 3027','esp':'Application bytes:','emu':'tamapoke-emu.exe',
      'dex':'TamaPoke-3.4.0-dex.exe','shiny':'TamaPoke-3.4.0-shiny.exe'}
for name,marker in logs.items():
    source=R/f'build/release-340-{name}.log';content=source.read_text(encoding='utf8',errors='replace')
...
for apk,abis in [('Android-Full-debug.apk',{'arm64-v8a','x86_64'}),('WearOS-GalaxyWatch4-9-debug.apk',{'arm64-v8a','armeabi-v7a'})]:
    with zipfile.ZipFile(A/('TamaPoke-3.4.0-'+apk)) as z:
        ...
        assert b'3.4.0' in z.read('lib/arm64-v8a/libtamapoke.so')
with zipfile.ZipFile(A/'TamaPoke-3.4.0-ESP32-Web-Installer.zip') as z:
    assert z.testzip() is None
    assert z.read('firmware/app.bin')==(R/'web/firmware/app.bin').read_bytes()
watch=A/'TamaPoke-3.4.0-Galaxy-Watch4-9-Install-Guide-KO.pdf'
assert digest(watch)==digest(R/'docs/guides/TamaPoke-Galaxy-Watch4-9-Install-Guide-KO.pdf')
guide=A/'TamaPoke-3.4.0-Play-Guide-KO.pdf';pdf=PdfReader(str(guide));assert len(pdf.pages)==25
assert all(p.extract_text().strip() for p in pdf.pages)
for p in (R/'build/3.4.0').glob('guide-contact-*.png'):shutil.copy2(p,Q/p.name)
report=dict(version='3.4.0',runtime_suites=53,sd_fault_checks=10,device_tested=False,full_team_survival=True,active_reserve_arrangements=30,lan_auto_switch_sync=True,wild_shiny_percent=1,shiny_egg_dex_priority_removed=True,explicit_capture_result=True,dex_rewards=True,dex_goal_percent=50,shiny_cap_percent=15,legend_cap_percent=21,disk_persistence=True,android_sleep_restart=True,companion_growth=True,compact_ivs=True,box_sort=True,box_direct_release=True,explicit_box_exchange=True,
    ...
    android_version_code=3026,wear_version_code=3027,guide_pages=25,watch_guide_unchanged=True,
    artifacts={p.name:info(p) for p in files},
    private_emulators={name:info(R/f'build/3.4.0/{name}/TamaPoke-3.4.0-{name}.exe') for name in ['dex','shiny']},
