"""Verify the exact public 3.5.2 bundle and record local evidence."""
import hashlib,json,re,zipfile
from pathlib import Path
from pypdf import PdfReader
R=Path(__file__).resolve().parents[1];V='3.5.2';A=R/'build/release'/V;Q=R/'docs/qa'/V
Q.mkdir(parents=True,exist_ok=True)
def sha(p):
    with p.open('rb') as f:return hashlib.file_digest(f,'sha256').hexdigest()
markers={'focused':'PASS: swap_recovery,active_swap,companion_growth,trade_ui,android_lifecycle,ending_recovery,forms_ui,korean',
         'android':'versionCode 3032','wear':'versionCode 3033','esp':'Published verified Korean components',
         'emu':'tamapoke-emu.exe','dex':'TamaPoke-3.5.2-dex.exe','shiny':'TamaPoke-3.5.2-shiny.exe',
         'watch-installer':'30 checks passed.','sd-store':'PASS factory reset','tail':'PASS: box_sort,revive'}
for name,marker in markers.items():
    s=(R/f'build/release-352-{name}.log').read_text(encoding='utf8',errors='replace')
    ...
    (Q/f'{name}.log').write_text(s,encoding='utf8')
for name in ('runtime','remaining'):
    f=R/f'build/release-352-{name}.log'
    if f.exists():(Q/f'windows-{name}.log').write_text(f.read_text(encoding='utf8',errors='replace'),encoding='utf8')
...
assert all('3.5.1' not in p.extract_text() for p in watch.pages)
report=dict(version=V,local_focused_suites=8,runtime_suite_total=56,windows_full_run='blocked by WinError 4551 at i18n; see CI for complete run',
            ...
            android_version_code=3032,wear_version_code=3033,artifacts={n:{'bytes':(A/n).stat().st_size,'sha256':sha(A/n)} for n in sorted(expected)})
