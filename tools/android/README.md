...
The APK is written to `build/android/TamaPoke-2.0.0-Android-Full-debug.apk`.
...
Growth and the clock screen follow Android's system date/time directly, while
the header battery icon follows the device battery percentage and charging
state. The
...
The output is `build/android/TamaPoke-2.0.0-WearOS-GalaxyWatch4-9-debug.apk`.
