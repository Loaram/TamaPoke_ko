"""Verify the exact public 3.6.0 bundle and record local evidence."""
import hashlib,json,re,zipfile,sys
from pathlib import Path
from pypdf import PdfReader
from capture_guide_data import probability, capture_rows, EXAMPLES
R=Path(__file__).resolve().parents[1];V='3.6.0';A=R/'build/release'/V;Q=R/'docs/qa'/V
Q.mkdir(parents=True,exist_ok=True)
def sha(p):
    with p.open('rb') as f:return hashlib.file_digest(f,'sha256').hexdigest()
markers={'android':'versionCode 3036','wear':'versionCode 3037','esp':'Published verified Korean components',
         'emu':'tamapoke-emu.exe','watch-installer':'30 checks passed.'}
for name,marker in markers.items():
    s=(R/f'build/release-360-{name}.log').read_text(encoding='utf8',errors='replace')
...
assert 'PASS: 59 runtime suites including collection capture bonus, recovery clocks and durable individual transfer' in runtime
assert 'MATRIX rate_count_cases=251648' in runtime
...
report=dict(version=V,runtime_suite_total=59,runtime_environment='GitHub Ubuntu',windows_full_run='capture_dex executable blocked by WinError 4551; no security changes',
            rate_count_cases=251648,capture_simulation_draws=5000000,simulation_source_equivalence=True,ci_url=ci['url'],
            ...
            android_version_code=3036,wear_version_code=3037,artifacts={n:{'bytes':(A/n).stat().st_size,'sha256':sha(A/n)} for n in sorted(expected)})
(Q/'build-results.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
print('PASS: 59 CI suites, 251648 rate/count cases, 5000000 captures, guide table and unchanged verified code')
