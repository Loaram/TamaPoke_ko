"""Verify the exact public 3.8.0 bundle and record local evidence."""
import hashlib,json,re,zipfile,sys
from pathlib import Path
from pypdf import PdfReader
from capture_guide_data import probability, capture_rows, EXAMPLES
R=Path(__file__).resolve().parents[1];V='3.8.0';A=R/'build/release'/V;Q=R/'docs/qa'/V
Q.mkdir(parents=True,exist_ok=True)
def sha(p):
    with p.open('rb') as f:return hashlib.file_digest(f,'sha256').hexdigest()
markers={'android':'versionCode 3048','wear':'versionCode 3049','esp':'Published verified Korean components',
         'emu':'tamapoke-emu.exe','watch-installer':'30 checks passed.'}
for name,marker in markers.items():
    s=(R/f'build/release-380-{name}.log').read_text(encoding='utf8',errors='replace')
...
claim_log=(R/'build/380-daily-egg-final.log').read_text(encoding='utf8',errors='replace')
assert 'PASS: daily_egg' in claim_log and not re.search(r'^FAIL|Traceback',claim_log,re.M)
assert claim_log.count('67 write boundaries, 0 failures')==2
report=dict(version=V,save_version=7,record_bytes=72,android_version_code=3048,wear_version_code=3049,
  runtime_ci_url='https://github.com/Loaram/TamaPoke_ko/actions/workflows/verify.yml',
  runtime_ci_note='65 suites must pass before publication; see Actions for live final results.',
  daily_menu_eggs=2,runaway_auto_egg_separate=True,crash_write_boundaries=134,
  local_regression_suites=8,local_regression_names=['daily_egg','balance','forms','daily_rewards','ending_recovery','companion_growth','release','daily_rewards_ui'],
  local_retire_test='Windows WinError4551 blocked; CI required',installer_checks=30,device_tested=False,
