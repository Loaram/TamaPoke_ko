...
with no art anywhere should be kept out of the egg pool. Approved alternate-form
fallbacks from pmd_forms.py count as usable art. Every earlier member of its
evolution family is excluded too, so an egg cannot begin a line that can lead to
a blank sprite.
...
import sys
import re
from pmd_forms import sprite_subpath
...
RAW = 'https://raw.githubusercontent.com/PMDCollab/SpriteCollab/master/sprite/%04d%s/AnimData.xml'
...
def packable(dexes):
    ...
    def has(d):
        ...
        sub = sprite_subpath(d, False)
        suffix = '/' + sub if sub else ''
        r = subprocess.run(['curl', '-sI', '-o', os.devnull, '-w', '%{http_code}',
                            RAW % (d, suffix)], capture_output=True, text=True)
        code = r.stdout.strip()
        if code == '200':
            return d, True
        if code == '404':
