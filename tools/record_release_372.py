"""Verify the exact public 3.7.2 bundle and record local evidence."""
import hashlib,json,re,zipfile,sys
from pathlib import Path
from pypdf import PdfReader
from capture_guide_data import probability, capture_rows, EXAMPLES
R=Path(__file__).resolve().parents[1];V='3.7.2';A=R/'build/release'/V;Q=R/'docs/qa'/V
Q.mkdir(parents=True,exist_ok=True)
def sha(p):
    with p.open('rb') as f:return hashlib.file_digest(f,'sha256').hexdigest()
markers={'android':'versionCode 3044','wear':'versionCode 3045','esp':'Published verified Korean components',
         'emu':'tamapoke-emu.exe','watch-installer':'30 checks passed.'}
for name,marker in markers.items():
    s=(R/f'build/release-372-{name}.log').read_text(encoding='utf8',errors='replace')
...
report=dict(version=V,save_version=7,record_bytes=72,android_version_code=3044,wear_version_code=3045,
  runtime_ci_url='https://github.com/Loaram/TamaPoke_ko/actions/runs/34192939937',
  ...
  local_regression_suites=9,installer_checks=30,device_tested=False,
