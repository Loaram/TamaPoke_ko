"""Verify and archive the 3.0.1 public artifact set, without publishing it."""
import argparse,hashlib,json,re,shutil,subprocess,sys,zipfile
from pathlib import Path
from pypdf import PdfReader
R=Path(__file__).resolve().parents[1];Q=R/'docs/qa/3.0.1';A=R/'build/release/3.0.1'
...
logs={'runtime':'44 runtime suites','sd-store':'PASS factory reset','android':'versionCode 3014',
      'wear':'versionCode 3015','esp':'Application bytes: 2237440','emu':'tamapoke-emu.exe',
      'dex':'TamaPoke-3.0.1-dex.exe','shiny':'TamaPoke-3.0.1-shiny.exe'}
for name,marker in logs.items():
    source=R/f'build/release-301-{name}.log';content=source.read_text(encoding='utf8',errors='replace')
...
for apk,abis in [('Android-Full-debug.apk',{'arm64-v8a','x86_64'}),('WearOS-GalaxyWatch4-9-debug.apk',{'arm64-v8a','armeabi-v7a'})]:
    with zipfile.ZipFile(A/('TamaPoke-3.0.1-'+apk)) as z:
        ...
        assert b'3.0.1' in z.read('lib/arm64-v8a/libtamapoke.so')
with zipfile.ZipFile(A/'TamaPoke-3.0.1-ESP32-Web-Installer.zip') as z:
    assert z.testzip() is None
    assert z.read('firmware/app.bin')==(R/'web/firmware/app.bin').read_bytes()
watch=A/'TamaPoke-3.0.1-Galaxy-Watch4-9-Install-Guide-KO.pdf'
assert digest(watch)==digest(R/'docs/guides/TamaPoke-Galaxy-Watch4-9-Install-Guide-KO.pdf')
guide=A/'TamaPoke-3.0.1-Play-Guide-KO.pdf';pdf=PdfReader(str(guide));assert len(pdf.pages)==22
assert all(p.extract_text().strip() for p in pdf.pages)
for p in (R/'build/3.0.1').glob('guide-contact-*.png'):shutil.copy2(p,Q/p.name)
report=dict(version='3.0.1',runtime_suites=44,sd_fault_checks=10,device_tested=False,box_direct_release=True,explicit_box_exchange=True,
    ...
    android_version_code=3014,wear_version_code=3015,guide_pages=22,watch_guide_unchanged=True,
    artifacts={p.name:info(p) for p in files},
    private_emulators={name:info(R/f'build/3.0.1/{name}/TamaPoke-3.0.1-{name}.exe') for name in ['dex','shiny']},
