...
배포 준비 후속: 최종 ESP·PDF·설치 도우미를 갱신했고 공식 9개 파일은 [배포 검증 기록](../3.7.2/build-results.json)으로 연결합니다. 아래 내용은 업로드 승인 전 로컬 전수검수 기록입니다. 서버에서 전체 그림 검사 실행 후 라이츄 폼 ID 테스트 오타(10002 → 10202)를 바로잡았습니다. 게임 코드는 변경하지 않았으며 최종 공개는 전체 CI 통과 후 진행합니다.
