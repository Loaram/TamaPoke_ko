...
// Match the current font for both half-width centering and full-width labels.
static const char *combatantName(const Combatant &c) {
  if (gLang == LANG_KO && c.dex >= 1 && c.dex <= DEX_COUNT) {
    char original[sizeof(c.name)];
    snprintf(original, sizeof(original), "%s", DEX_TBL[c.dex].name);
    if (!strcmp(original, c.name)) return localName(DEX_TBL[c.dex].name);
  }
  return c.name;
}

static int textWidthFactor(const char *s, int factor) {
  return gfx->textWidth(s) * factor / (6 * gfx->fontScale());
}
...
bool menuOpen = false;
bool formsOpen = false, formPreviewDirty = true;
uint16_t formTarget = 0;
uint8_t formPage = 0;
PmdMon formPreview;
FormId monFormFor = 0, btlFormFor[2] = {0,0};
...
uint16_t boxDetail = 0;        // box slot + 1 whose sheet is open
...
static void drawGymArt(const char *key, int x, int y, int scale = 1) {
  const GymArt *a = gymArt(key);
  if (!a) return;
  for (int r=0;r<a->size;r++) for(int c=0;c<a->size;c++) {
    uint8_t v=a->idx[r*a->size+c];
    if(v!=255) gfx->fillRect(x+c*scale,y+r*scale,scale,scale,a->pal[v]);
  }
}
...
uint8_t gymRegion = 0;
uint8_t btlRegion = 0;
bool gymShield = false, btlShield = false;
#define TRAINER_AT(i) trainerAt(gymRegion, (i), gymShield)
#define BTL_TRAINER_AT(i) trainerAt(btlRegion, (i), btlShield)
...
static bool gymUnlocked(uint8_t idx, bool hard) {
  return idx == 0 || pet.hasBadge(gymRegion, trainerBadgeIndex(gymRegion,idx - 1,gymShield), hard);
}
...
void ensureMon() {
  ...
  if (pet.speciesId >= 1 && pet.speciesId <= DEX_COUNT) {
    pmd.loadForm(pet.speciesId, formFind(pet.speciesId,pet.form)?pet.form:0, pet.shiny);          // principal: PMD
    if (!pmd.loaded && !pet.form) mon.load(pet.speciesId, pet.shiny);  // respaldo: B/N
  }
}
...
void loop() {
  ...
  if (pet.endedKind != CER_NONE && !partyPick) {
    ...
    if (party.hasEndedMon(pet.endedMon) || party.add(pet.endedMon) || party.boxAdd(pet.endedMon)) {
      snprintf(partyBannerName, sizeof(partyBannerName), "%s",
               pet.endedMon.nick[0] ? pet.endedMon.nick : localName(DEX_TBL[pet.endedMon.dex].name));
      partyBannerUntil = now + 3500;
      pet.acknowledgeEnding();
      sfxPlay(SFX_MEDAL);
    } else {
      partyPick = true;
      partyOpen = true;
...
void handleSerial() {
  ...
  if (line == "HATCH") {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      Serial.printf("%d:%s(r%u) ", d, localName(DEX_TBL[d].name), DEX_TBL[d].rarity);
    }
    Serial.println();
    Serial.println("DONE");
    ...
    if (n >= 1 && dex >= 1 && dex <= DEX_COUNT) {
      ...
      Serial.printf("battle vs %s Lv.%u\n", localName(DEX_TBL[dex].name), btlFoe.level);
    } else {
    ...
    // The serial exporter is rare: do not reserve a second 32-KiB DRAM buffer
    // for the lifetime of the game. ESP allocates this temporary block in PSRAM.
    uint8_t *buf=(uint8_t*)ps_malloc(SAVE_MAX_BYTES);
    if(!buf){Serial.println("EXPORT MEMORY");return;}
    size_t n = saveExport(buf, SAVE_MAX_BYTES);
    ...
    for (int i = 0; i < PARTY_SLOTS; i++) {
      ...
      else Serial.printf(" %s%s(nv%u)", localName(DEX_TBL[m.dex].name), m.shiny ? "*" : "", m.level);
    }
    Serial.println();
    Serial.println("DONE");
  } else if (line == "REG") {
...
void renderMonSheet(const PartyMon &m, bool fromBox) {
  ...
  snprintf(head, sizeof(head), "%s%s Lv.%u", m.shiny ? "*" : "",
           m.nick[0] ? m.nick : localName(d.name), (unsigned)m.level);
  gfx->setTextColor(d.accent);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(head, 6), 40);
  gfx->print(head);
  gfx->fillRoundRect(342,57,58,20,5,UI_TRACK);
  gfx->setTextColor(UI_INK);gfx->setTextSize(1);gfx->setCursor(350,62);gfx->print(gLang==LANG_KO?"폼":"Form");
  char ty[72];
  ...
  gfx->setTextSize(1);
  gfx->setCursor(CX - textWidthFactor(ty, 3), 64);
  gfx->print(ty);
  ...
  gfx->drawRoundRect(PDET_L_X, PDET_BTN_Y, liveW, PDET_BTN_H, 10, UI_INK);
  gfx->setTextColor(leftOk ? UI_BG_DAY : 0x8410);
  gfx->setTextSize(fromBox?1:2);
  gfx->setCursor(PDET_L_X + liveW / 2 - textWidthFactor(leftLbl, fromBox?3:6),
                 PDET_BTN_Y + PDET_BTN_H / 2 - (fromBox?4:8));
  gfx->print(leftLbl);
  if(fromBox) {
  ...
  gfx->setTextSize(1);
  gfx->setCursor(PDET_R_X + PDET_R_W / 2 - textWidthFactor(T(S_RELEASE_BTN), 3),
  ...
  if (!leftOk) {
    ...
    gfx->setCursor(CX-textWidthFactor(T(S_ACTIVE_SWAP_HINT),3),PDET_BTN_Y-14);
    gfx->print(T(S_ACTIVE_SWAP_HINT));
  }
  gfx->setTextColor(0x6B4D);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(T(S_BACK), 6), 404);
  gfx->print(T(S_BACK));
  ...
  if (releaseConfirm) {
    char q[120];
    snprintf(q, sizeof(q), T(S_RELEASE_FMT), m.nick[0] ? m.nick : localName(d.name));
...
void partyTap(int16_t x, int16_t y) {
  ...
  if ((y >= 372 && y <= 416 && x >= 133 && x <= 333) || y < 34) {
    if (partyPick) {                 // declined the swap: the pet is let go
      if(!pet.acknowledgeEnding())return;
      partyPick = false;
    }
    partyOpen = false;
    sfxPlay(SFX_TAP);
...
void onTap(int16_t x, int16_t y) {
...
#endif
  if (playerOpen) {
    if (playerPage == 7 && y >= 100 && y <= 150) {
      gymShield = !gymShield; sfxPlay(SFX_TAP); return;
    }
  ...
  if (gymOpen) {
    if (gymRegion == 7 && y >= 16 && y < 60 && x >= 105 && x <= 361) {
      gymShield = !gymShield; sfxPlay(SFX_TAP); return;
    }
  ...
  if (movePickOpen) {
    ...
    for (uint8_t i = 0; i < MOVE_PICK_PER_PAGE; i++) {
      ...
      MoveId before[MOVE_SLOTS];memcpy(before,tgt,sizeof(before));
      for (int s = 0; s < MOVE_SLOTS; s++)
        if (tgt[s] == all[idx] && s != movePickSlot) tgt[s] = tgt[movePickSlot];
      tgt[movePickSlot] = all[idx];
      if (movePickParty || movePickBox) {
        if(!party.save()){memcpy(tgt,before,sizeof(before));sfxPlay(SFX_DENY);return;}
      } else pet.saveNow();
      movePickOpen = false;
      return;
    }
  ...
  if (partyOpen) {
    if(!releaseConfirm && x>=342 && x<=400 && y>=57 && y<=77) {
      if(boxOpen && boxDetail) {formOpenTarget(7+boxDetail-1);return;}
      if(!boxOpen && partyDetail) {formOpenTarget(partyDetail);return;}
    }
    partyTap(x, y);
    return;
  }
  ...
  if (cardOpen) {
    if(cardPage==1 && x>=116 && x<=350 && y>=320 && y<=362) {formOpenTarget(0);return;}
...
void renderStarterSelect() {
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(t, 6), 68);
  gfx->print(t);
  for (int i = 0; i < starterCountShown(pet.region); i++) {
...
void render() {
  ...
  if (pet.ceremony) {
    ...
    const char *msg = (pet.ceremony == CER_FAREWELL) ? T(S_FAREWELL)
                      : (pet.ceremony == CER_RUNAWAY) ? T(S_RUNAWAY)
                                                      : T(S_GOODBYE);
    drawHeader(localName(d.name), d.accent, msg);
    drawCeremony();
    gfx->flush();
    return;
  }
  ...
  if (pet.isEgg()) {
    ...
    if (pet.eggRarity() >= R_RARO) {
      ...
      gfx->setTextSize(2);
      gfx->setCursor(CX - textWidthFactor(rar, 6), 316);
      gfx->print(rar);
    }
    char reg[72];
    ...
    gfx->setTextSize(2);
    gfx->setCursor(CX - textWidthFactor(reg, 6), 344);
    gfx->print(reg);
    ...
    const DexEntry &d = DEX_TBL[pet.speciesId];
    char name[84];
    const char *base = pet.nick[0] ? pet.nick : localName(d.name);
  ...
  if (confirmUntil) {
    if (millis() > confirmUntil) {
      ...
      char q[84];
      snprintf(q, sizeof(q), T(S_RELEASE_FMT), localName(DEX_TBL[pet.speciesId].name));
      gfx->setTextColor(UI_INK);
      gfx->setTextSize(2);
      gfx->setCursor(CX - textWidthFactor(q, 6), 196);
      gfx->print(q);
      snprintf(q, sizeof(q), gLang == LANG_KO ? "오늘 남은 작별 %u/3" : "Farewells left today: %u/3", pet.farewellsRemaining());
      gfx->setTextSize(1);
      gfx->setCursor(CX - textWidthFactor(q, 3), 226);
      gfx->print(q);
      gfx->setTextSize(2);
      gfx->fillRoundRect(118, 252, 100, 52, 12, pet.farewellsRemaining() ? UI_BAR_OK : UI_TRACK);
      gfx->setTextColor(UI_WHITE);
      gfx->setCursor(118 + (100 - textWidthFactor(T(S_YES), 12)) / 2, 270);
      gfx->print(T(S_YES));
      gfx->fillRoundRect(248, 252, 100, 52, 12, UI_BAR_BAD);
      gfx->setCursor(248 + (100 - textWidthFactor(T(S_NO), 12)) / 2, 270);
      gfx->print(T(S_NO));
    }
  }
  ...
  if (partyBannerUntil) {
    if (millis() > partyBannerUntil) {
      ...
      gfx->setTextColor(UI_WHITE);
      gfx->setTextSize(2);
      gfx->setCursor(CX - textWidthFactor(b, 6), 206);
      gfx->print(b);
    }
  }
...
void renderSack() {
  ...
  if (sackOverUntil) {
    ...
    gfx->setTextSize(4);
    gfx->setCursor(CX - textWidthFactor(b, 12), 150);
    gfx->print(b);
    char g[64];
    snprintf(g, sizeof(g), T(S_STR_GAIN_FMT), sackGain);
    gfx->setTextColor(UI_BAR_BAD);
    gfx->setTextSize(3);
    gfx->setCursor(CX - textWidthFactor(g, 9), 210);
    gfx->print(g);
    gfx->setTextSize(2);
    if (sackNewHi && sackHits > 0) {
      ...
      gfx->setTextColor(ink);
      gfx->setCursor(CX - textWidthFactor(r, 6), 256);
      gfx->print(r);
    }
    gfx->flush();
    return;
  ...
  gfx->setTextColor(ink);
  gfx->setTextSize(6);
  gfx->setCursor(CX - textWidthFactor(buf, 18), 268);
  gfx->print(buf);

  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(T(S_HIT_FAST), 6), 322);
...
void renderGame() {
  ...
  gfx->setTextColor(ink);
  gfx->setTextSize(4);
  gfx->setCursor(CX - textWidthFactor(buf, 12), 30);
  gfx->print(buf);
  char rec[64];
  snprintf(rec, sizeof(rec), T(S_REC_FMT), pet.gameHi);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(rec, 6), 76);
  gfx->print(rec);
  for (int i = 0; i < 3; i++) {
...
void renderClock() {
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(3);
  gfx->setCursor(CX - textWidthFactor(T(S_SET_TIME), 9), 44);
  ...
  gfx->setTextSize(2);
  gfx->setCursor(34 + (96 - textWidthFactor(sl, 12)) / 2, LANG_PILL_Y + 8);
  gfx->print(sl);
  ...
  // master -- this is how loud it is when it is on, and 0 is silent.
  {
    ...
    gfx->setTextSize(1);
    gfx->setCursor(210 + (56 - textWidthFactor(vl, 6)) / 2, LANG_PILL_Y + 4);
    gfx->print(vl);
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(LANG_PILL_X + (LANG_PILL_W - textWidthFactor(lp, 12)) / 2, LANG_PILL_Y + 8);
  gfx->print(lp);
...
void drawCelebration() {
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(3);
  gfx->setCursor(CX - textWidthFactor(l1, 9), 176);
  gfx->print(l1);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(l2, 6), 212);
  gfx->print(l2);
}
...
void drawMedalBadge(int x, int y, int i) {
  ...
  gfx->setTextSize(2);
  gfx->setCursor(x + (100 - textWidthFactor(medalLabel(i), 12)) / 2, y + 5);
...
void renderCardProfile() {
  const DexEntry &d = DEX_TBL[pet.speciesId];
  const char *nm = pet.nick[0] ? pet.nick : localName(d.name);
  char head[78];
  ...
  int hlen = uiTextWidth(head, 3);
  int hts = (hlen <= 198) ? 3 : 2;
  gfx->setTextSize(hts);
  gfx->setCursor(CX - uiTextWidth(head, hts) / 2, hts == 3 ? 34 : 40);
  gfx->print(head);
  if (pet.nick[0]) {  // especie real bajo el apodo
    gfx->setTextColor(0x6B4D);
    gfx->setTextSize(2);
    gfx->setCursor(CX - (uiTextWidth(localName(d.name), 2) + 24) / 2, 64);
    gfx->printf("(%s)", localName(d.name));
  }
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(info, 6), 296);
  gfx->print(info);

  gfx->setTextColor(0x6B4D);
  gfx->setCursor(CX - textWidthFactor(T(S_RENAME_HINT), 6), 332);
...
void renderCardStats() {
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(3);
  gfx->setCursor(CX - textWidthFactor(T(S_BATTLE), 9), 44);
  ...
  const DexEntry de = formDex(pet.speciesId,pet.form);
  char ty[72];
  ...
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(ty, 6), 76);
  gfx->print(ty);
  ...
  gfx->fillRoundRect(116,320,234,42,10,UI_TRACK);
  gfx->setTextColor(UI_INK);gfx->setTextSize(2);
  const char *fl=gLang==LANG_KO?"폼체인지":"Forms";
  gfx->setCursor(CX-uiTextWidth(fl,2)/2,332);gfx->print(fl);
...
int drawTypeChip(int x, int y, uint8_t type) {
  ...
  gfx->fillRoundRect(x, y, w, 18, 4, typeColor(type));
  gfx->setTextSize(1);
...
void drawMoveRow(int y, MoveId mv, bool highlight, int16_t dex, FormId form) {
  gfx->fillRoundRect(70, y, 326, 50, 12, highlight ? UI_BAR_WARN : UI_BG_DAY);
  gfx->drawRoundRect(70, y, 326, 50, 12, UI_INK);
  if (!mv) {
    gfx->setTextColor(0x6B4D);
    gfx->setTextSize(2);
    gfx->setCursor(CX - textWidthFactor(T(S_MOVE_EMPTY), 6), y + 17);
  ...
  bool stab = hasStab(dex, formMoveType(dex,form,mv), form) && m.cat != MC_STATUS;
  int cw = drawTypeChip(82, y + 29, formMoveType(dex,form,mv));
  if (stab) {
    gfx->setTextColor(DEX_TBL[dex].accent);
    gfx->setTextSize(1);
    gfx->setCursor(82 + cw + 6, y + 33);
    gfx->print(localName("STAB"));
  }
  char pw[64];
  ...
  gfx->setTextSize(1);
  gfx->setCursor(384 - textWidthFactor(pw, 6), y + 33);
  gfx->print(pw);
}
...
void renderCardMoves() {
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(3);
  gfx->setCursor(CX - textWidthFactor(T(S_MOVES), 9), 44);
  gfx->print(T(S_MOVES));
  for (int i = 0; i < MOVE_SLOTS; i++) drawMoveRow(MOVE_ROW_Y(i), pet.moves[i], false, pet.speciesId,pet.form);
  gfx->setTextColor(0x6B4D);
  gfx->setTextSize(1);
  gfx->setCursor(CX - textWidthFactor(T(S_MOVE_TAP), 3), 340);
...
uint8_t learnableFor(int16_t dex, uint8_t lvl, MoveId *out, uint8_t max) {
  return formLearnableList(dex,0,lvl,out,max);
}
...
uint8_t learnableList(MoveId *out, uint8_t max) {
  if(movePickBox){const auto &m=party.box[movePickBox-1];return formLearnableList(m.dex,m.form,(uint8_t)m.level,out,max);}
  if (movePickParty) {
    const PartyMon &m = party.slots[movePickParty - 1];
    return formLearnableList(m.dex,m.form,(uint8_t)m.level,out,max);
  }
  return pet.isEgg() ? 0 : formLearnableList(pet.speciesId,pet.form,pet.level(),out,max);
}

MoveId *pickTargetMoves() {
  if(movePickBox)return party.box[movePickBox-1].moves;
  return movePickParty ? party.slots[movePickParty - 1].moves : pet.moves;
}
int16_t pickTargetDex() {
  if(movePickBox)return party.box[movePickBox-1].dex;
...
void renderMovePick() {
  ...
  for (uint8_t i = 0; i < MOVE_PICK_PER_PAGE; i++) {
    ...
    drawMoveRow(MOVE_PICK_Y(i), all[idx], all[idx] == pickTargetMoves()[movePickSlot], pickTargetDex(),movePickBox?party.box[movePickBox-1].form:movePickParty?party.slots[movePickParty-1].form:pet.form);
  }
  if (pages > 1) {
    char pg[16];
  ...
  gfx->setTextColor(0x6B4D);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(T(S_BACK), 6), 402);
  gfx->print(T(S_BACK));
  gfx->flush();
}
...
static void btlSyncSprite(uint8_t who, const Combatant &c) {
  ...
  if (btlPmdDex[who] == key && btlFormFor[who] == c.form && btlPmd[who].loaded) return;
  btlFormFor[who]=c.form;
  btlPmd[who].unload();
  btlPmdDex[who] = 0;
  if (c.dex < 1 || c.dex > DEX_COUNT) return;
  if (btlPmd[who].loadForm(c.dex,c.form,c.shiny)) btlPmdDex[who] = key;   // NOT (uint8_t): Hoenn runs past 255
}
...
static void btlNarrate(const Combatant &actor, const Combatant &target, const TurnLog &lg) {
  ...
  if (lg.charged) { btlSay(T(S_BTL_USED), combatantName(actor), localName(MOVE_TBL[lg.move].name)); return; }
  if (lg.move) btlSay(T(S_BTL_USED), combatantName(actor), localName(MOVE_TBL[lg.move].name));
  if (lg.missed) { btlSay(T(S_BTL_MISS), combatantName(actor)); return; }
  ...
  if (lg.inflicted) {
    ...
    if (lg.inflicted < 7) btlSay(T(S_BTL_STATUS), combatantName(target), T(AIL_STR[lg.inflicted]));
  }
  if (lg.targetFainted) btlSay(T(S_BTL_FAINT), combatantName(target));
}
...
static void foeFromSpecies(Combatant &c, int16_t dex, uint8_t lvl, uint8_t iv, uint16_t form = 0) {
  Pet foe;
...
uint8_t squadCap(uint8_t idx, bool hard) {
  ...
  return hard ? TRAINER_AT(idx).count : TRAINER_TEAM_MAX;
}
...
void startTrainerBattle(uint8_t idx, bool hard) {
  ...
  btlShield = gymShield;
  const Trainer &tr = BTL_TRAINER_AT(idx);
  uint8_t top = 0;
  ...
  btlHard = hard;
  btlFoeAt = 0;
  const Trainer &t = BTL_TRAINER_AT(idx);
  foeFromSpecies(btlFoe, t.team[0].dex, t.team[0].level, hard ? HARD_IV : EASY_IV, t.team[0].form);
  btlFoe.npcType1=t.team[0].type1; btlFoe.npcType2=t.team[0].type2;
  btlMsgCount = 0;
  btlOver = false;
  btlWon = false;
  btlMenu = 0;
...
static void btlApplyResult() {
  ...
  btlMsgCount = 0;
  if (r.hostMove) btlSay(T(S_BTL_USED), combatantName(btlFoe), localName(MOVE_TBL[r.hostMove].name));
  if (r.guestMove) btlSay(T(S_BTL_USED), combatantName(btlYou), localName(MOVE_TBL[r.guestMove].name));
  ...
  if (btlYou.fainted()) {
    ...
    btlSay(T(S_BTL_FAINT), combatantName(btlYou));
  }
  if (btlFoe.fainted()) {
    btlFaintUntil[1] = now + BTL_FAINT_MS;
    btlSay(T(S_BTL_FAINT), combatantName(btlFoe));
  }
}
...
static void btlResolve(MoveId yourMove) {
  ...
  if (btlLink) {
    ...
    if (LINK_ACT_IS_SWITCH(act)) {
      ...
      if (to < btlFoeSquadN && to != btlFoeAt && !btlFoeSquad[to].fainted()) {
        ...
        btlLungeUntil[1] = btlHitUntil[1] = btlFaintUntil[1] = 0;
        btlEnterUntil[1] = now + BTL_ENTER_MS;
        btlSay(T(S_BTL_SENDS), lan.peerName, combatantName(btlFoe));
        foeSwitched = true;
      }
      foeMove = 0;
    } else {
  ...
  if (btlFoe.fainted() || btlYou.fainted()) {
    ...
    if (btlWon && btlTrainer >= 0 && !pet.hasBadge(btlRegion, trainerBadgeIndex(btlRegion,btlTrainer,btlShield), btlHard)) {
      pet.winBadge(btlRegion, trainerBadgeIndex(btlRegion,btlTrainer,btlShield), btlHard);
      btlNewBadge = true;
    }
...
static void btlSide(int tx, int ty, int sx, int sy, const Combatant &c, uint8_t who) {
  ...
  char l[84];
  snprintf(l, sizeof(l), "%s Lv.%u", combatantName(c), c.level);
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(1);
  gfx->setCursor(tx, ty);
  gfx->print(l);
  ...
  if (who == 0) {                 // your own numbers, as the games do
    ...
    gfx->setTextColor(UI_INK);
    gfx->setCursor(tx + 140 - textWidthFactor(hp, 6), ty + 28);
    gfx->print(hp);
  }
  ...
  if (who==1 && btlRegion==8 && btlTrainer==5 && c.dex==849) {
    drawGymArt("NPC_849_LOW",sx-8+ox,sy+14+oy); return;
  }
  ...
  // Official stills are NPC-only: do not unlock these species in eggs/explore.
  if (who==1 && btlTrainer>=0 && gymNpcHasArt(c.dex)) {
    char key[16]; snprintf(key,sizeof(key),"NPC_%d",c.dex);
    drawGymArt(key,sx-8+ox,sy+14+oy); return;
  }
  if(c.form) {drawFormMini(c.dex,c.form,c.shiny,sx+24+ox,sy+48+oy);return;}
...
void renderWin() {
  const Trainer &t = BTL_TRAINER_AT(btlTrainer);
  gfx->fillScreen(RGB565_BLACK);
  gfx->fillCircle(CX, CY, 231, UI_BG_DAY);

  gfx->setTextColor(UI_BAR_WARN);
  gfx->setTextSize(3);
  gfx->setCursor(CX - textWidthFactor(T(S_BTL_WIN), 9), 54);
  gfx->print(T(S_BTL_WIN));

  char l[120];
  snprintf(l, sizeof(l), T(S_BTL_BEAT), localName(t.name));
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(l, 6), 96);
  gfx->print(l);
  ...
  if (btlTrainer < TRAINER_GYMS) {
    ...
    if (a) {
      ...
      gfx->setCursor(CX - textWidthFactor(n, 9), by - 11);
      gfx->print(n);
    }
    if (btlNewBadge) {
      gfx->setTextColor(UI_BAR_OK);
      gfx->setTextSize(2);
      gfx->setCursor(CX - textWidthFactor(T(S_BTL_NEWBADGE), 6), 286);
      gfx->print(T(S_BTL_NEWBADGE));
    }
  }
  snprintf(l, sizeof(l), T(S_BADGES_FMT), pet.gymBadgeCountIn(btlRegion, btlHard, btlShield));
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(l, 6), 316);
  gfx->print(l);
  ...
  if (btlTrainGain) {
    ...
    gfx->setTextColor(UI_BAR_OK);
    gfx->setTextSize(2);
    gfx->setCursor(CX - textWidthFactor(l, 6), 344);
    gfx->print(l);
  } else if (btlPetIn && btlTrainer >= 0) {
    gfx->setTextColor(0x6B4D);
    gfx->setTextSize(1);
    gfx->setCursor(CX - textWidthFactor(T(S_WIN_MAXED), 3), 348);
    gfx->print(T(S_WIN_MAXED));
  }

  gfx->setTextColor(0x6B4D);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(T(S_BACK), 6), 380);
  gfx->print(T(S_BACK));
  gfx->flush();
}
...
void renderBattle() {
  ...
  if (lanWait && !btlMsgCount) {
    ...
    gfx->setCursor(CX - textWidthFactor(w, 3), BTL_GRID_Y + 40);
    gfx->print(w);
    ...
    for (uint8_t i = 0; i < btlMsgCount && i < 4; i++) {
      gfx->setCursor(CX - textWidthFactor(btlMsg[i], 3), BTL_GRID_Y + 14 + i * 18);
      gfx->print(btlMsg[i]);
    }
    gfx->setTextColor(0x6B4D);
    gfx->setCursor(CX - 30, BTL_GRID_Y + 84);
    gfx->print(localName("tap..."));
  } else if (btlMenu == 0) {
    ...
    gfx->setTextColor(UI_INK);
    gfx->setTextSize(2);
    gfx->setCursor(CX - textWidthFactor(T(S_FIGHT), 6), BTL_GRID_Y + 14);
    gfx->print(T(S_FIGHT));
    ...
    for (int i = 0; i < 2; i++) {
      ...
      gfx->setTextColor(UI_INK);
      gfx->setTextSize(2);
      gfx->setCursor(x + (BTL_CELL_W - textWidthFactor(low[i], 12)) / 2, y + 14);
      gfx->print(low[i]);
    }
  } else if (btlMenu == 2) {
    drawBtlBack();
    ...
    for (int i = 0; i < MOVE_SLOTS; i++) {
      ...
      gfx->print(localName(MOVE_TBL[mv].name));
      ...
      int cw = drawTypeChip(x + 10, y + 26, formMoveType(btlYou.dex,btlYou.form,mv));
      if (hasStab(btlYou.dex, formMoveType(btlYou.dex,btlYou.form,mv),btlYou.form) &&
...
static void btlDoSwap() {
  ...
  if (btlSwapWho == 1 && btlLink) {
    ...
    foeFromSpecies(btlFoe, t.team[btlFoeAt].dex, t.team[btlFoeAt].level,
                   btlHard ? HARD_IV : EASY_IV, t.team[btlFoeAt].form);
    btlFoe.npcType1=t.team[btlFoeAt].type1; btlFoe.npcType2=t.team[btlFoeAt].type2;
    btlHpShown[1] = btlFoe.maxHp;
    btlSyncSprite(1, btlFoe);
    btlLungeUntil[1] = btlHitUntil[1] = btlFaintUntil[1] = 0;
    btlEnterUntil[1] = now + BTL_ENTER_MS;
...
static void drawBtlBack() {
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(T(S_BACK), 6), BTL_BACK_Y + 14);
  gfx->print(T(S_BACK));
}
...
static void renderPlayerBadges() {
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(3);
  gfx->setCursor(CX - textWidthFactor(tn, 9), 40);
  gfx->print(tn);
  ...
  if (playerBadgeRegion != 0) {
    const char *rn = playerBadgeRegion == 7 ? localName(gymShield ? "GALAR SHIELD" : "GALAR SWORD") : localName(TRAINER_SETS[playerBadgeRegion].region);
    gfx->setTextColor(UI_INK);
    gfx->setTextSize(2);
    gfx->setCursor(CX - textWidthFactor(rn, 6), 120);
    gfx->print(rn);
    ...
    const char *an = localName(AVATARS[pet.avatar % AVATAR_COUNT].name);
    gfx->setTextColor(UI_INK);
    gfx->setTextSize(1);
    gfx->setCursor(CX - textWidthFactor(an, 3), 143);
    gfx->print(an);
    gfx->setTextColor(0x6B4D);
    gfx->setCursor(CX - textWidthFactor(T(S_AVATAR_HINT), 3), 154);
  ...
  for (int i = 0; i < TRAINER_GYMS; i++) {
    ...
    if (!a) {                 // no art for this region: a filled disc, won is won
      gfx->fillCircle(bx, by, 14, typeColor(trainerAt(playerBadgeRegion,i,gymShield).type));
...
static void renderPlayerMedals() {
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(3);
  gfx->setCursor(CX - textWidthFactor(head, 9), 44);
  gfx->print(head);
  ...
  for (int i = 0; i < MED_COUNT; i++) {
    ...
    gfx->setTextSize(2);
    gfx->setCursor(x + (180 - textWidthFactor(medalLabel(i), 12)) / 2, y + 6);
    gfx->print(medalLabel(i));
    gfx->setTextSize(1);
    gfx->setCursor(x + (180 - textWidthFactor(medalDesc(i), 6)) / 2, y + 30);
  ...
  gfx->setTextColor(0x6B4D);
  gfx->setTextSize(1);
  gfx->setCursor(CX - textWidthFactor(tot, 3), 344);
  gfx->print(tot);
}
...
static void drawPickCell(uint8_t n, int x, int y, uint8_t capLvl) {
  ...
  if (n == 0) {
    ...
    nm = pet.nick[0] ? pet.nick : localName(DEX_TBL[dex].name);
  } else {
    const PartyMon &m = party.slots[n - 1];
    dex = m.dex; lvl = m.level; shiny = m.shiny;
    nm = m.nick[0] ? m.nick : localName(DEX_TBL[dex].name);
  }
  ...
  FormId selectedForm=n==0?pet.form:party.slots[n-1].form;
  if(selectedForm) drawFormMini(dex,selectedForm,shiny,x+29,y+32);
  else if (th) drawThumb(th, x - 12, y - 6, 2, !on);
  ...
  const DexEntry d = formDex(dex,n==0?pet.form:party.slots[n-1].form);
...
void renderPick() {
  ...
  if (pickTrainer == PICK_LAN) {
    ...
    const Trainer &t = TRAINER_AT(pickTrainer);
    for (int k = 0; k < t.count; k++)
      if (t.team[k].level > top) top = t.team[k].level;
    snprintf(head, sizeof(head), "%s  Lv.%u x%u", localName(t.name), top, t.count);
  }
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(head, 6), 44);
  gfx->print(head);
  char sub[84];
  snprintf(sub, sizeof(sub), T(S_PICK_FMT), pickChosen(), cap);
  gfx->setTextSize(1);
  gfx->setTextColor(pickChosen() > cap ? UI_BAR_BAD : UI_TRACK);
  gfx->setCursor(CX - textWidthFactor(sub, 3), 68);
  gfx->print(sub);
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(PICK_BACK_X + (PICK_BTN_W - textWidthFactor(T(S_BACK), 12)) / 2,
                 PICK_GO_Y + 14);
  gfx->print(T(S_BACK));
  ...
  gfx->setTextSize(2);
  gfx->setCursor(PICK_GO_X + (PICK_BTN_W - textWidthFactor(T(S_FIGHT), 12)) / 2,
                 PICK_GO_Y + 14);
  gfx->print(T(S_FIGHT));
  gfx->flush();
}
...
void renderLan() {
  ...
  if (lan.state == LINK_OFF || lan.state == LINK_REFUSED ||
    ...
    if (lan.peerName[0]) {
      gfx->setTextColor(UI_INK);
      gfx->setTextSize(2);
      gfx->setCursor(CX - textWidthFactor(lan.peerName, 6), 130);
      gfx->print(lan.peerName);
    }
    snprintf(l, sizeof(l), T(S_LAN_VS), lan.theirsN);
    gfx->setTextColor(UI_INK);
    gfx->setTextSize(2);
    gfx->setCursor(CX - textWidthFactor(l, 6), 150);
    gfx->print(l);
    gfx->fillRoundRect(120, 220, 226, 56, 12, UI_BAR_OK);
    gfx->drawRoundRect(120, 220, 226, 56, 12, UI_INK);
    gfx->setTextColor(UI_BG_DAY);
    gfx->setCursor(CX - textWidthFactor(T(S_FIGHT), 6), 240);
    gfx->print(T(S_FIGHT));
    ...
    if (lan.peerName[0]) {
      gfx->setTextColor(UI_INK);
      gfx->setTextSize(2);
      gfx->setCursor(CX - textWidthFactor(lan.peerName, 6), 140);
      gfx->print(lan.peerName);
    }
...
void renderGyms() {
  ...
  char title[84];
  snprintf(title, sizeof(title), "%s %s", localName(TRAINER_SETS[gymRegion % GYM_REGIONS].region),
           gymRegion == 7 ? localName(gymShield ? "SHIELD" : "SWORD") : T(S_GYMS));
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(title, 6), 42);
  gfx->print(title);
  ...
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(dif, 6), GYMDIF_Y + 14);
  gfx->print(dif);

  for (int i = 0; i < GYM_ROWS; i++) {
    uint8_t idx = gymPage * GYM_ROWS + i;
    if (idx >= TRAINER_COUNT) break;
    const Trainer &t = TRAINER_AT(idx);
    int y = GYM_ROW_Y(i);
    bool done = pet.hasBadge(gymRegion, trainerBadgeIndex(gymRegion,idx,gymShield), gymHard);
    ...
    gfx->setTextSize(2);
    bool portrait = gymArt(t.name) != nullptr;
    if (portrait) drawGymArt(t.name,78,y+6);
    gfx->setCursor(portrait ? 118 : 84, y + 8);
    gfx->print(localName(t.name));
    gfx->setTextSize(1);
    gfx->setTextColor(0x6B4D);
    gfx->setCursor(portrait ? 118 : 84, y + 28);
    gfx->print(open_ ? localName(t.place) : T(S_LOCKED));
    ...
    gfx->setCursor(384 - textWidthFactor(lv, 6), y + 28);
    gfx->print(lv);
    if (done) {
      gfx->setTextColor(UI_BAR_OK);
...
static void drawEggRegion() {
  char l[72];
  snprintf(l, sizeof(l), "%s >", localName(pet.regionName()));
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(EGGREG_X + (EGGREG_W - textWidthFactor(l, 12)) / 2, EGGREG_Y + 9);
  gfx->print(l);
  gfx->setTextColor(0x6B4D);
  gfx->setTextSize(1);
  gfx->setCursor(CX - textWidthFactor(T(S_EGG_REGION), 3), EGGREG_Y + EGGREG_H + 6);
...
static void renderRegionPick(uint8_t mode) {
  ...
  for (uint8_t row = 0; row < RPICK_PER_PAGE; row++) {
    ...
    const char *nm = forGyms ? localName(TRAINER_SETS[i].region) : localName(REGIONS[i].name);
    ...
    if (!open)
      ...
      snprintf(sub, sizeof(sub), T(S_BADGES_FMT), pet.gymBadgeCountIn(i, gymHard, gymShield));
    ...
    if (sub[0]) {
      gfx->setTextColor(0x6B4D);
      gfx->setTextSize(2);
      gfx->setCursor(RPICK_X + RPICK_W - 18 - textWidthFactor(sub, 12), y + 22);
      gfx->print(sub);
    }
  }
  if (forGyms) {
...
void renderLearn() {
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(1);
  gfx->setCursor(CX - textWidthFactor(head, 3), 48);
  gfx->print(head);
  gfx->setTextColor(DEX_TBL[pet.speciesId].accent);
  gfx->setTextSize(3);
  gfx->setCursor(CX - textWidthFactor(localName(MOVE_TBL[mv].name), 9), 66);
  gfx->print(localName(MOVE_TBL[mv].name));

  for (int i = 0; i < MOVE_SLOTS; i++) drawMoveRow(LEARN_ROW_Y(i), pet.moves[i], false, pet.speciesId,pet.form);

  gfx->fillRoundRect(70, LEARN_SKIP_Y, 326, 44, 12, UI_TRACK);
  gfx->drawRoundRect(70, LEARN_SKIP_Y, 326, 44, 12, UI_INK);
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(T(S_LEARN_SKIP), 6), LEARN_SKIP_Y + 14);
...
void renderCardMedals() {
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(3);
  gfx->setCursor(CX - textWidthFactor(head, 9), 48);
  gfx->print(head);
...
void renderCardProgress() {
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(3);
  gfx->setCursor(CX - textWidthFactor(T(S_PROGRESS), 9), 44);
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(nx, 6), by + 32);
  gfx->print(nx);

  // estado de evolucion
  gfx->setTextColor(0x6B4D);
  gfx->setCursor(CX - textWidthFactor(T(S_EVO_LABEL), 6), 230);
  ...
  gfx->setCursor(CX - textWidthFactor(ms, 6), 312);
  gfx->print(ms);
}
...
void renderCard() {
  ...
  gfx->setTextColor(0x6B4D);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(T(S_BACK), 6), 398);
  gfx->print(T(S_BACK));
  gfx->flush();
}
...
void drawMenu() {
  ...
  for (int i = 0; i < MENU_ROWS; i++) {
    ...
    gfx->setTextColor(UI_INK);
    gfx->setTextSize(2);
    gfx->setCursor(CX - textWidthFactor(lbl, 6), y + MENU_ROW_H / 2 - 8);
    gfx->print(lbl);
  }
}
...
void renderTrain() {
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(T(S_TRAIN), 6), TRAIN_Y + 20);
  gfx->print(T(S_TRAIN));
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(1);
  gfx->setCursor(CX - textWidthFactor(T(S_TR_DEF_HINT), 3), TRAIN_Y + TRAIN_H - 22);
  gfx->print(T(S_TR_DEF_HINT));
  gfx->flush();   // without this the panel never updates and the screen freezes
}

// ---------- the box ----------
// Shared navigation stays clear of the six cells and the round screen bezel.
#define BOX_NAV_Y 338
#define BOX_NAV_H 44
#define BOX_PICK_SIZE 10
void boxPickerStep(int delta) {
  int next = (int)boxPickerGroup + delta;
  if (next < 0 || next >= (BOX_PAGES + BOX_PICK_SIZE - 1) / BOX_PICK_SIZE) return;
  boxPickerGroup = (uint8_t)next;
  sfxPlay(SFX_TAP);
}

void drawBoxNavigation(bool picker) {
  char page[24];
  int current = picker ? boxPickerGroup : boxPage;
  int pages = picker ? (BOX_PAGES + BOX_PICK_SIZE - 1) / BOX_PICK_SIZE : BOX_PAGES;
  if (picker) {
    unsigned first = boxPickerGroup * BOX_PICK_SIZE + 1;
    unsigned last = first + BOX_PICK_SIZE - 1;
    if (last > BOX_PAGES) last = BOX_PAGES;
    snprintf(page, sizeof(page), "%u-%u/%u", first, last, (unsigned)BOX_PAGES);
  } else snprintf(page, sizeof(page), "%u/%u", (unsigned)boxPage + 1, (unsigned)BOX_PAGES);
  const int xs[3] = {84, 160, 316}, widths[3] = {66, 146, 66};
  const char *labels[3] = {"<", page, ">"};
  for (int i=0;i<3;i++) {
    bool disabled = (i==0 && current==0) || (i==2 && current==pages-1);
    gfx->fillRoundRect(xs[i],BOX_NAV_Y,widths[i],BOX_NAV_H,10,disabled || (picker && i==1) ? UI_TRACK : UI_WHITE);
    gfx->drawRoundRect(xs[i],BOX_NAV_Y,widths[i],BOX_NAV_H,10,UI_INK);
    gfx->setTextColor(disabled ? 0x8410 : UI_INK);gfx->setTextSize(2);
    gfx->setCursor(xs[i]+widths[i]/2-textWidthFactor(labels[i],6),BOX_NAV_Y+14);
    gfx->print(labels[i]);
  }
}

void renderBoxPagePicker() {
  gfx->fillScreen(RGB565_BLACK);gfx->fillCircle(CX,CY,231,UI_BG_DAY);
  const char *title = gLang==LANG_KO ? "페이지 바로 이동" : "GO TO PAGE";
  gfx->setTextColor(UI_INK);gfx->setTextSize(2);
  gfx->setCursor(CX-textWidthFactor(title,6),56);gfx->print(title);
  for (int i=0;i<BOX_PICK_SIZE;i++) {
    int page=boxPickerGroup*BOX_PICK_SIZE+i;
    if(page>=BOX_PAGES) break;
    int x=88+(i%2)*150,y=100+(i/2)*48;
    gfx->fillRoundRect(x,y,140,42,10,page==boxPage ? UI_BAR_OK : UI_WHITE);
    gfx->drawRoundRect(x,y,140,42,10,UI_INK);
    char label[12];snprintf(label,sizeof(label),"%u",(unsigned)page+1);
    gfx->setTextColor(page==boxPage ? UI_WHITE : UI_INK);gfx->setTextSize(2);
    gfx->setCursor(x+70-textWidthFactor(label,6),y+13);gfx->print(label);
  }
  drawBoxNavigation(true);
  gfx->setTextColor(UI_INK);gfx->setTextSize(2);
  gfx->setCursor(CX-textWidthFactor(T(S_BACK),6),400);gfx->print(T(S_BACK));gfx->flush();
}

void boxPagePickerTap(int16_t x,int16_t y) {
  if (y>=392) {boxPagePicker=false;sfxPlay(SFX_TAP);return;}
  if (y>=BOX_NAV_Y && y<BOX_NAV_Y+BOX_NAV_H) {
    if(x>=84 && x<150) boxPickerStep(-1);
    else if(x>=316 && x<382) boxPickerStep(1);
    return;
  }
  for(int i=0;i<BOX_PICK_SIZE;i++) {
    int xx=88+(i%2)*150,yy=100+(i/2)*48;
    int page=boxPickerGroup*BOX_PICK_SIZE+i;
    if(x>=xx && x<xx+140 && y>=yy && y<yy+42 && page<BOX_PAGES) {
      boxPage=(uint8_t)page;boxPagePicker=false;sfxPlay(SFX_TAP);return;
    }
  }
}
...
void renderBox() {
  ...
  if (boxSwapFrom) {
    ...
    snprintf(sub, sizeof(sub), T(S_BOX_SWAP),
             p.empty() ? "-" : (p.nick[0] ? p.nick : localName(DEX_TBL[p.dex].name)));
    gfx->setTextColor(UI_BAR_WARN);
    gfx->setTextSize(1);
    gfx->setCursor(CX - textWidthFactor(sub, 3), 64);
    gfx->print(sub);
  }
  for (uint8_t i = 0; i < BOX_PER_PAGE; i++) {
    uint16_t idx = boxPage * BOX_PER_PAGE + i;
    ...
    gfx->print(m.nick[0] ? m.nick : localName(DEX_TBL[m.dex].name));
    char l[64];
...
void boxTap(int16_t x, int16_t y) {
  ...
  if (boxDetail) {
    ...
    for(int i=0;i<MOVE_SLOTS;i++)if(x>=70 && x<=396 && y>=78+i*52 && y<=128+i*52) {
      movePickBox=boxDetail;movePickParty=0;movePickSlot=i;movePickPage=0;movePickOpen=true;return;
    }
    if (monSheetBtn(x, y, true)) {          // TO PARTY
      ...
      if (free < 0) {
        ...
        boxSel=boxDetail; boxDetail=partyDetail=boxSwapFrom=0;
        boxOpen=false; sfxPlay(SFX_TAP); return;
      }
  ...
  if (y>=BOX_NAV_Y && y<BOX_NAV_Y+BOX_NAV_H) {
    if (x>=84 && x<150) {
      if(boxPage>0) {boxPage--;sfxPlay(SFX_TAP);}
    } else if(x>=316 && x<382) {
      if(boxPage+1<BOX_PAGES) {boxPage++;sfxPlay(SFX_TAP);}
    } else if(x>=160 && x<306) {
      boxPickerGroup=boxPage/BOX_PICK_SIZE;boxPagePicker=true;sfxPlay(SFX_TAP);
    }
    return;
  }
  for (uint8_t i = 0; i < BOX_PER_PAGE; i++) {
...
void drawPartySlot(int i, int x, int y) {
  ...
  if (m.empty()) {
    gfx->setTextColor(0x8410);
    gfx->setTextSize(2);
    gfx->setCursor(x + (PARTY_CELL_W - textWidthFactor(T(S_PARTY_EMPTY), 12)) / 2,
                   y + PARTY_CELL_H / 2 - 8);
    gfx->print(T(S_PARTY_EMPTY));
    return;
  }
  ...
  if (m.shiny) {
    gfx->setTextColor(UI_BAR_WARN);
    gfx->setCursor(x + 62 + textWidthFactor(nm, 6) + 3, y + 18);
    gfx->print("*");
  }
  char lv[64];
...
void renderParty() {
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(3);
  gfx->setCursor(CX - textWidthFactor(head, 9), 42);
  gfx->print(head);
  ...
  if (!partyPick) {
    ...
    gfx->setTextColor(UI_INK);
    gfx->setTextSize(2);
    gfx->setCursor(233 - textWidthFactor(bl, 6), BOXBTN_Y + 12);
    gfx->print(bl);
  }
  ...
  if (boxSel) {
    ...
    snprintf(sw, sizeof(sw), T(S_BOX_SWAP),
             b.nick[0] ? b.nick : localName(DEX_TBL[b.dex].name));
    gfx->setTextColor(UI_BAR_WARN);
    gfx->setTextSize(1);
    gfx->setCursor(CX - textWidthFactor(sw, 3), 72);
    gfx->print(sw);
  }
  ...
  if (partyPick) {
    gfx->setTextColor(UI_BAR_BAD);
    gfx->setTextSize(1);
    gfx->setCursor(CX - textWidthFactor(T(S_PARTY_FULL), 3), 74);
  ...
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(ex, 6), PARTYCLOSE_Y + 14);
  gfx->print(ex);
  gfx->flush();
}
...
void renderKeyboard() {
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(T(S_NAME), 6), 56);
  gfx->print(T(S_NAME));
  // buffer actual
...
void renderGallery() {
  if (galleryDetail) {  // vista detalle: se redibuja siempre (animada)
    ...
    snprintf(head, sizeof(head), "N.%03d %s%s", galleryDetail,
             pet.isShinyRegistered(galleryDetail) ? "*" : "", reg ? localName(d.name) : "???");
    gfx->setTextColor(reg ? d.accent : UI_INK);
    int glen = uiTextWidth(head, 3);
    int gts = (glen <= 234) ? 3 : 2;  // auto-encoge nombres largos (no caben a t3)
    gfx->setTextSize(gts);
    gfx->setCursor(CX - uiTextWidth(head, gts) / 2, gts == 3 ? 56 : 60);
    gfx->print(head);
  ...
  gfx->setTextColor(0x6B4D);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(pg, 6), 428);
  gfx->print(pg);
  gfx->flush();
}
...
void drawHeader(const char *name, uint16_t nameColor, const char *msg) {
  ...
  gfx->setTextSize(uiTextWidth(name, 3) <= 300 ? 3 : 2);
  gfx->setCursor(CX - gfx->textWidth(name) / 2, 52);
  gfx->print(name);
  gfx->setTextColor(inkColor());
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(msg, 6), 90);
  gfx->print(msg);
}
...
void drawConfirmPanel(const char *q, const char *sub1, const char *sub2,
  ...
  gfx->setTextColor(t1);
  gfx->setCursor(CX - textWidthFactor(o1, 6), CONFIRM_B1_Y + 18);
  gfx->print(o1);
  gfx->fillRoundRect(CONFIRM_BTN_X, CONFIRM_B2_Y, CONFIRM_BTN_W, CONFIRM_BTN_H, 12, c2);
  gfx->setTextColor(t2);
  gfx->setCursor(CX - textWidthFactor(o2, 6), CONFIRM_B2_Y + 18);
  gfx->print(o2);
}
...
void drawEvolveButton() {
  ...
  gfx->setCursor(CX - textWidthFactor(t, 9), y + h / 2 - 11);
  gfx->print(t);
}
...
void drawFarewellButton() {
  ...
  gfx->setTextColor(UI_INK);
  gfx->setTextSize(2);
  gfx->setCursor(CX - textWidthFactor(buf, 6), y + h / 2 - 8);
  gfx->print(buf);
}
...
void drawPet() {
  ...
  int fi = pet.form ? -1 : flashIdxForDex(pet.speciesId);
  if (fi < 0) {
    ...
    gfx->setCursor(CX - textWidthFactor(l1, 6), PET_CY - 4);
    gfx->print(l1);
    const char *l2 = T(S_LOAD_SPRITES);
    gfx->setCursor(CX - textWidthFactor(l2, 6), PET_CY + 20);
    gfx->print(l2);
    return;
  }
