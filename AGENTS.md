...
- Build/emulator/web checks have passed for ko.1.0.7. Hardware tests remain unverified until actual results are recorded. See the hardware test issue template and `docs/HARDWARE_TEST.ko.md`.
- For every numbered release, also build two private emulator variants: full normal Pokedex with `-dex` and full shiny Pokedex with `-shiny` (for example `ko.1.0.7-dex` and `ko.1.0.7-shiny`). Never publish or attach either executable to GitHub releases or Pages. Build them with `tools/build_emulator.py --full-dex` and `tools/build_emulator.py --full-shiny`; they use `tamapoke-dex.nvs` and `tamapoke-shiny.nvs` instead of the normal emulator save.
